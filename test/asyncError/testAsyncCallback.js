describe(require('../__fixtures/utils/test_helper').create().testName(__filename, 3), function() {
  ...
  it('tests the __performDataRequest function with __asyncErrorCallback overRidden. Supposed to Produce error', function(done) {
    ...
    happnClient.__asyncErrorCallback = function(error, callback) { callback(error)};
    var count = 0
    var arr = [];
    for (i = 0; i < 1000000; i++) {
      arr.push(i)
    }
    async.each(arr, (item,itemCB) => {
      setTimeout(() => happnClient.__performDataRequest('/test/path', 'set', {
        test: 'data'
