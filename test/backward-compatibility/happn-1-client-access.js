describe(require('../__fixtures/utils/test_helper').create().testName(__filename, 3),
  function () {
    ...
    context('resources access testing', function () {
      ...
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/all_access'
      ] = {
          actions: ['*']
        };
      testGroup.permissions[
      ...
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/on_all/*'
      ] = {
          actions: ['on']
        };
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/remove'
      ] = {
          actions: ['remove']
        };
      testGroup.permissions[
      ...
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/get_all/*'
      ] = {
          actions: ['get']
        };
      testGroup.permissions[
      ...
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/set_all/*'
      ] = {
          actions: ['set']
        };
      testGroup.permissions[
        '/TEST/a7_eventemitter_security_access/' + test_id + '/all/*'
      ] = {
          actions: ['*']
        };
      testGroup.permissions[
            ...
            testClient.set(
              ...
              function (e, result) {
                if (!e)
                  return done(
                    new Error(
                      'you just set data that you shouldnt have permissions to set'
                    )
                  );
                expect(e.toString()).to.be('AccessDenied: unauthorized');
                done();
          ...
          function (e, setResult) {
            ...
            testClient.get(
              ...
              function (e, result) {
                ...
                testClient.get(
                  '/TEST/a7_eventemitter_security_access/dodge/' +
                  test_id +
                  '/get',
                  {},
                  function (e, result) {
                    if (!e)
                      return done(
                        new Error(
                          'you managed to get data which you do not have permissions for'
                        )
                      );
                    expect(e.toString()).to.be('AccessDenied: unauthorized');
                    done();
              ...
              function (e, result) {
                ...
                testClient.on(
                  ...
                  function (e) {
                    if (!e)
                      return done(
                        new Error('this should not have been allowed...')
                      );
                    expect(e.toString()).to.be('AccessDenied: unauthorized');
                    done();
                  }
      ...
      it('checks against a permission that doesnt exist', function (done) {
        testClient.get(
          '/TEST/a7_eventemitter_security_access/whatevs' +
          test_id +
          '/get_all/' +
          test_id,
          {},
      ...
      it('deletes the test user, tests we are notified about the session closure, then have no access', function (done) {
        ...
        serviceInstance.services.security.users.deleteUser(
          addedTestuser,
          function (e) {
            if (e) return done(e);
          }
