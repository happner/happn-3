describe(require('../__fixtures/utils/test_helper').create().testName(__filename, 3), function () {
  ...
  after('should kill the services and client', function (callback) {
    ...
    console.log('disconnecting testClient, this will take 50 seconds and should log a timeout error...');
  ...
  context('resources access testing', function () {
    ...
    it('deletes the test user, tests we are notified about the session closure, then have no access', function (done) {
      ...
      testClient.onSystemMessage(function (eventType, data) {
        if (eventType == 'server-side-disconnect' && data.reason == 'security directory update: user deleted') {
          done();
        }
      });
