describe(
  ...
  function() {
      ...
      async.eachSeries(
        ...
        function(serviceName, eachServiceCB) {
          ...
          if (serviceName == 'session') {
            ...
            return happnMock.services[serviceName].initializeCaches(
              eachServiceCB
            );
          }

          if (!happnMock.services[serviceName].initialize)
    ...
    it('should test the session filtering capability', function(done) {
      ...
      var foundItem = sift(
        serviceConfig.services.security.config.profiles[0].session,
        [testSession]
      );

      expect(foundItem.length).to.be(1);

      var notFoundItem = sift(
        serviceConfig.services.security.config.profiles[0].session,
        [testSessionNotFound]
      );
      ...
      var foundInGroupItem = sift(
        serviceConfig.services.security.config.profiles[0].session,
        [testSessionNotFound, testSession]
      );
      ...
      var foundItemProfile1 = sift(
        serviceConfig.services.security.config.profiles[1].session,
        [testSession1]
      );
      ...
      var foundItemProfile2 = sift(
        serviceConfig.services.security.config.profiles[2].session,
        [testSession2, testSession1]
      );

      expect(foundItemProfile2.length).to.be(1);

      expect(foundItemProfile2[0].user.groups.TRUSTED_DEVICES).to.not.be(null);
      expect(foundItemProfile2[0].user.groups.TRUSTED_DEVICES).to.not.be(
        undefined
      );
    ...
    it('should test the __prepareLogin method, password', function(done) {
      ...
      clientInstance.performRequest = function(
        path,
        action,
        data,
        options,
    ...
    it('should test the __prepareLogin method, digest', function(done) {
      ...
      clientInstance.__performSystemRequest = function(
        action,
        data,
        options,
        cb
          ...
          if (action == 'login') {
            if (data.digest) {
              if (nonce_requests[data.nonce] !== null) {
                ...
                happnMock.services.crypto.verify(
                  data.nonce,
                  data.digest,
                  data.publicKey
                );
                ...
                callback(
                  new Error(
                    'could not find nonce request for nonce: ' + data.nonce
                  )
                );
            }
          }
    ...
    it('tests the security checkpoints token usage limit', function(done) {
      ...
      cacheInstance.initialize({}, function(e) {
        ...
        checkpoint.initialize({}, securityService, function(e) {
          ...
          checkpoint.__checkUsageLimit(
            testSession,
            testSession.policy[1],
            function(e, ok) {
              if (e) return done(e);
    ...
    it('tests the security services createAuthenticationNonce and verifyAuthenticationDigest methods', function(done) {
      ...
      clientInstance.__ensureCryptoLibrary(function(e) {
        ...
        mockServices(function(e, happnMock) {
          ...
          expect(happnMock.services.security.config.defaultNonceTTL).to.be(
            60000
          );

          happnMock.services.security.createAuthenticationNonce(
            mockSession,
            function(e, nonce) {
              if (e) return done(e);
                ...
                happnMock.services.security.verifyAuthenticationDigest(
                  ...
                  function(e) {
                    expect(e.toString()).to.be(
                      'Error: nonce expired or public key invalid'
                    );
    ...
    it('should test the policy ms settings', function(done) {
      ...
      securityService
        ...
        .then(function() {
          ...
          expect(
            securityService.__cache_Profiles[0].policy.inactivity_threshold
          ).to.be(2000);
          expect(
            securityService.__cache_Profiles[1].policy.inactivity_threshold
          ).to.be(60000 * 60 * 48);
          done();
        })
        .catch(done);
    ...
    it('tests the processLogin method, session dropped while logging in', function(done) {
      getService(
        ...
        function(e, instance) {
          ...
          instance.services.security.login = function(
            credentials,
            sessionId,
            request,
            callback
          ...
          instance.services.security
            ...
            .catch(function(e) {
              expect(e.toString()).to.be(
                'Error: session with id 1 dropped while logging in'
              );
