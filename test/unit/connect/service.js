describe(require('../../__fixtures/utils/test_helper').create().testName(__filename, 3), function () {
  ...
  async function newConnect(config){
    ...
    return new Promise(function(resolve, reject){
      ...
      var instance = new Connect({logger:{
  ...
  it('should load the connect service up, middlewares have the correct happn', async () => {
    ...
    expect(connect1.middleware['system'].happn.config).to.eql({secure:true, port:55001, services:{
      transport:{
        config:{
    ...
    expect(connect2.middleware['system'].happn.config).to.eql({secure:true, port:55002, services:{
      transport:{
        config:{
    ...
    expect(connect3.middleware['system'].happn.config).to.eql({secure:true, port:55003, services:{
      transport:{
        config:{
