describe('01a_websockets_embedded_sanity_encrypted_payloads', function() {
  ...
  it('the listener should pick up a single published event, eventemitter listening', function(callback) {
    ...
    try {
      ...
      socketClient.on(
        ...
        function(message) {
          expect(
            socketClient.state.events[
              '/SET@/e2e_test1/testsubscribe/data/event'
            ]
          ).to.equal(undefined);
          callback();
        },
    ...
    socketClient.on(
      ...
      function(data) {
        ...
        socketClient.off(variableDepthHandle, function(e) {
          ...
          expect(socketClient.state.listenerRefs[variableDepthHandle]).to.eql(
            undefined
          );
    ...
    socketClient.on(
      ...
      function(e, handle1) {
        socketClient.on(
          ...
          function(data) {},
          function(e, handle2) {
            expect(Object.keys(socketClient.state.listenerRefs).length).to.eql(
              2
            );
            ...
            socketClient.disconnect(function(e) {
              if (e) return done(e);
              expect(
                Object.keys(socketClient.state.listenerRefs).length
              ).to.eql(0);
              done();
            });
          }
              ...
              socketClient.off(handle1, function(e) {
                ...
                socketClient.set('/test/path/1/1', { set: 2 }, function(e) {
                  ...
                  expect(results).to.eql([
                    {
                      data: { set: 1 },
                      channel: '/ALL@/test/path/1/**',
                      path: '/test/path/1/1'
                    },
                    {
                      data: { set: 1 },
                      channel: '/ALL@/test/path/**',
                      path: '/test/path/1/1'
                    },
                    {
                      data: { set: 2 },
