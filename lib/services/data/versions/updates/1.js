...
Update1.prototype.backup = function (records) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    async.eachSeries(records, function (group, groupCB) {

      report.started++;

      _this.updater.dataService.upsert('/_SYSTEM/DATABASE/BACKUPS/1/GROUP/' + group.data.name, group.data, function(e){
...
Update1.prototype.update = function (records) {
  ...
  return new Promise(function (resolve, reject) {
      ...
      Object.keys(group.data.permissions).forEach(function(permissionPath){
        ...
        if (group.data.permissions[permissionPath].actions) {
          group.data.permissions[permissionPath].actions.forEach(function(action){
            permissions.push({permissionPath:permissionPath, action:action, groupName: group.data.name});
          });
        }
      });
    });
    ...
    async.eachSeries(permissions, function(permission, permissionCB){

      report.started++;

      _this.updater.dataService.upsert(['/_SYSTEM/_SECURITY/_PERMISSIONS', permission.groupName, permission.action, __escapePermissionsPath(permission.permissionPath)].join('/'), {
