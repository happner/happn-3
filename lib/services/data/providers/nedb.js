...
NedbProvider.prototype.initialize = function(callback){
  ...
  console.log('INITIALIZED:::', this.db);
...
NedbProvider.prototype.getPathCriteria = function(path){
  ...
  if (returnType == -1) return {'path': path}; //precise match

  if (returnType == 0) return {'path': {$regex: new RegExp(path.replace(/[*]/g, '.*'))}};//keys with any prefix ie. */joe/bloggs

  return {'path': {$regex: new RegExp('^' + path.replace(/[*]/g, '.*'))}};//keys that start with something but any suffix /joe/*/bloggs/*
...
NedbProvider.prototype.remove = function(criteria, callback){
  ...
  return this.db.remove(criteria, {multi: true}, callback);
};
