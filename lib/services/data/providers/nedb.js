...
NedbProvider.prototype.utils = utils;
...
NedbProvider.prototype.__wrapCallBackInSync = __wrapCallBackInSync;
...
function __wrapCallBackInSync(callback) {
  const _this = this;
  return function() {
    try {
      if (!_this.settings.filename || !_this.settings.syncFileOnChange)
        return callback(...arguments);
      fs.fsyncSync(_this.settings.filename);
      callback(...arguments);
    } catch (e) {
      callback(new Error(`failed syncing to storage device: ${e.message}`));
    }
  };
}
...
function upsert(path, setData, options, dataWasMerged, callback) {
  ...
  _this.db.update(
    ...
    _this.utils.clone(setParameters),
    options,
    this.__wrapCallBackInSync((err, response, created, upserted, meta) => {
      if (err) {
