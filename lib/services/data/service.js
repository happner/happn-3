...
function doRemove(message, callback) {
  ...
  if (message.request.path.indexOf('*') > -1) return callback(new Error('error removing item(s) on path ' + path + ', you can only remove a single item, no wildcards are allowed on the path'));
