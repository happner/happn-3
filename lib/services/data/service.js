...
DataService.prototype.initialize = function (config, callback) {
  ...
  _this.__initializeProviders(callback);
};

DataService.prototype.__initializeProviders = function(callback){
  ...
  async.eachSeries(_this.config.datastores, function(datastoreConfig, datastoreCallback){
    ...
    if (e) return _this.errorService.handleSystem(e, 'DataService', callback);
...
DataService.prototype.__detachProviderEvents = function(provider){
...
DataService.prototype.__attachProviderEvents = function(providerKey, provider){
  ...
  var providerEventsHandler = function(data){
    this.service.emit('provider-event', {eventName:this.eventName, eventData:data, provider:this.providerKey});
  };

  //only one event so far, but there could be more, use bind to ensure we emit the correct eventName etc.
  provider.on('compaction-successful', providerEventsHandler.bind({service:_this, providerKey:providerKey, eventName:'compaction-successful'}));
...
DataService.prototype.saveTag = function (path, tag, data, callback) {
  ...
  if (!data) {
    ...
    return _this.getOneByPath(path, null, function (e, found) {
      ...
      return _this.errorService.handleSystem(new Error('Attempt to tag something that doesn\'t exist in the first place'), 'DataService', constants.ERROR_SEVERITY.LOW);
    });
  }
...
DataService.prototype.filter = function(criteria, data, callback){
  ...
  try{
    ...
    this.errorService.handleSystem(new Error('Filter of resultset failed', e), 'DataService', constants.ERROR_SEVERITY.MEDIUM, callback);
  }
};
...
DataService.prototype.get = function (path, parameters, callback) {
  ...
  try {
    ...
    provider.find(path, options, function (e, items) {
      ...
      if (e) return _this.errorService.handleSystem(new Error('Find failed on path ' + path, e), 'DataService', constants.ERROR_SEVERITY.MEDIUM, callback);
    ...
    this.errorService.handleSystem(new Error('Find failed on path ' + path, e), 'DataService', constants.ERROR_SEVERITY.MEDIUM, callback);
  }
};
...
DataService.prototype.remove = function (path, options, callback) {
  ...
  this.db(path).remove(path, function (err, removed) {
    ...
    if (err) return _this.errorService.handleSystem(new Error('error removing item(s) on path ' + path, err), 'DataService', constants.ERROR_SEVERITY.MEDIUM, callback);
...
DataService.prototype.compact = function (dataStoreKey,//what dataStore - if undefined we will compact all data stores
