...
DataService.prototype.__initializeProviders = function(callback){
  ...
  async.eachSeries(_this.config.datastores, function(datastoreConfig, datastoreCallback){
    ...
    if (e) return _this.errorService.handleSystem(e, 'DataService', constants.ERROR_SEVERITY.HIGH, callback);
  ...
  traverse(fields).forEach(function (value) {
    ...
    if (value) {
      ...
      if (typeof this.key == 'string') {
        ...
        if (this.key == '_meta.createdBy') {
          fields['created'] = value;
          return this.remove();
        }

        //look in the right place for modified
        if (this.key == '_meta.modifiedBy') {
          fields['modified'] = value;
          return this.remove();
        }
...
DataService.prototype.remove = function (path, options, callback) {
  ...
  this.db(path).remove(path, function (e, removed) {
    ...
    if (e) return callback(new Error('error removing item(s) on path ' + path, e));
