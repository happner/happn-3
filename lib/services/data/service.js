...
DataService.prototype.initialize = function (config, callback) {
  ...
  if (_this.config.datastores.length == 0)  _this.config.datastores.push(
    {
      name:'default',
      provider:'nedb',
      isDefault:true,
  ...
  console.log('_this.config.datastores:::', _this.config.datastores);
...
DataService.prototype.__doFind = function(path, searchOptions, sortOptions, callback){
  ...
  if (!sortOptions) return this.db(path)
    .find(provider.getPathCriteria(path), searchOptions, null, callback);

  this.db(path)
    .find(provider.getPathCriteria(path), searchOptions, this.parseFields(sortOptions), callback);
...
DataService.prototype.remove = function (path, options, callback) {
  ...
  provider.remove(provider.getPathCriteria(path), function (err, removed) {
...
DataService.prototype.startCompacting = function (dataStoreKey, interval, callback, compactionHandler) {
  try {
    ...
    _this.__iterateDataStores(dataStoreKey, function (key, dataStore, next) {
        ...
        _this.compact(key, next, compactionHandler);
      },
      callback);
...
DataService.prototype.compact = function (dataStoreKey, callback, compactionHandler) {

  var dataStore = this.datastores[dataStoreKey];

  this.__compacting[key] = dataStore;

  this.__compacting[key].startCompacting(interval, callback, compactionHandler);
};
