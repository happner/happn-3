...
function upsert(path, data, options, callback) {
  ...
  if (path.indexOf('*') > -1) return callback(new Error('error upserting item on path ' + path + ', you can only change a single item, no wildcards are allowed on the path'));
...
function remove (path, options, callback) {
  ...
  if (path.indexOf('*') > -1) return callback(new Error('error removing item(s) on path ' + path + ', you can only remove a single item, no wildcards are allowed on the path'));
...
function __initializeProviders(callback) {
  var _this = this;
