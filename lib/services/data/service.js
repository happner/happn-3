...
DataService.prototype.initialize = function (config, callback) {
  ...
  async.eachSeries(_this.config.datastores, function(datastoreConfig, datastoreCallback){
    ...
    try {
      ...
      else dataStoreInstance.patterns = [];
...
DataService.prototype.__getPullOptions = function(path, parameters){
  ...
  if (parameters){
    ...
    if (parameters.path_only || parameters.options.path_only) options.fields = {_meta: 1};

    if (parameters.fields || parameters.options.fields){

      options.fields = this.parseFields(parameters.options.fields || parameters.fields);
      options.fields._meta = 1;

    }

    if (parameters.sort || parameters.options.sort) options.sort = this.parseFields(parameters.sort || parameters.options.sort);
...
DataService.prototype.get = function (path, parameters, callback) {
  ...
  try {
    ...
    _this.db(path).find(path, options, function (e, items) {
      ...
      if (path.indexOf('*') == -1) {//this is a single item
        ...
        return callback(null, _this.transform(items[0], null, options.fields));
      }
      ...
      _this.filter(parameters.criteria, items, function(e, filtered){
        ...
        callback(null, _this.transformAll(filtered, options.fields));
      });
    });
...
DataService.prototype.upsert = function (path, data, options, callback) {
  ...
  console.log('internal upsert:::', setData);
...
DataService.prototype.__upsertInternal = function (path, setData, options, dataWasMerged, callback) {
  ...
  provider.upsert(path, setData, options, dataWasMerged, function(e, response, created, upsert, meta){
    ...
    if (dataWasMerged) {
      ...
      if (options.tag) return _this.insertTag(setData, options.tag, path, callback);//_this.saveTag(path, options.tag, setData, callback);// we have a prefetched object, and we want to tag it

      if (created) return callback(null, _this.transform(created));


      console.log('TRANSFORMING:::', setData, meta);
