...
function processGet(message, callback) {
  ...
  return this.get(message.request.path, message.request.options, (e, response) => {
    if (e) return this.errorService.handleSystem(e, 'DataService', constants.ERROR_SEVERITY.HIGH, (e) => {
      callback(e, message);
    });
...
function count(path, parameters, callback) {
  ...
  try{
    ...
    provider.count(path, options, function(e, items) {
...
function processRemove(message, callback) {
  ...
  return this.remove(message.request.path, message.request.options, (e, response) => {
    ...
    if (e) return this.errorService.handleSystem(e, 'DataService', constants.ERROR_SEVERITY.HIGH, function(e) {
      callback(e, message);
    });
...
function parseFields(fields) {
  //_this is essential here, as the traverse iterator function relies on its own scope
  var _this = this;
  var lastError;
    ...
    if (value != null) {
      ...
      if (typeof this.key == 'string') {
        ...
        if (this.key.indexOf('_data.') == 0) {
          if (!this.parent) fields[this.key.substring(1)] = value; //remove _
