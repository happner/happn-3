...
function initialize(config, callback) {
  ...
  if (_this.config.secure) _this.processStore = _this.processSecureStore;

  _this.__initializeProviders()
    .then(function(){
      return _this.__updateDatabaseVersions();
    })
    .then(function(){
      callback();
    })
...
function __updateDatabaseVersions(){
  ...
  return new Promise(function(resolve, reject){
    ...
    var versionUpdater = new VersionUpdater(_this, _this.happn.services.system);

    versionUpdater.analyzeDB(function(e, analysis){

      if (e) return reject(e);
      //make available so we can inspect in our integration tests
      _this.dbVersionAnalysis = analysis;
      ...
      versionUpdater.updateDB(analysis, function(logs){

        _this.log.warn('database upgrade from ' + analysis.currentDBVersion + ' to ' + analysis.moduleDBVersion);

        logs.forEach(function(log){
          _this.log.info(log.message);
        });
...
function processGet(message, callback) {
  ...
  return this.get(message.request.path, message.request.options, function (e, response) {
    ...
    if (e) return _this.errorService.handleSystem(new Error('processGet failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, function (e) {
      callback(e, message);
    });
...
function processRemove(message, callback) {
  ...
  return this.remove(message.request.path, message.request.options, function (e, response) {
    ...
    if (e) return _this.errorService.handleSystem(new Error('processRemove failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, function (e) {
      callback(e, message);
    });
...
function processStore(message) {
  ...
  if (message.request.options.noStore) return _this.processNoStore(message);

  return new Promise(function(resolve, reject){

    _this.upsert(message.request.path, message.request.data, message.request.options, function (e, response) {

      if (e) return _this.errorService.handleSystem(new Error('processStore failed', e), 'DataService', constants.ERROR_SEVERITY.HIGH, function (e) {
        reject(e);
      });
...
function processNoStore(message) {
  ...
  return new Promise(function(resolve, reject){
    try{
      message.response = _this.formatSetData(message.request.path, message.request.data);
      return resolve(message);
    }catch(e){
      reject(e);
    }
...
function __initializeProviders() {
  ...
  return new Promise(function(resolve, reject){
    ...
    async.eachSeries(_this.config.datastores, function (datastoreConfig, datastoreCallback) {
      ...
      _this.defaultProvider = _this.datastores[_this.defaultDatastore].provider;

      if (Object.keys(_this.dataroutes).length == 0){
        _this.db = function (path) {
          return _this.defaultProvider;
        };
      }
      else
        _this.db = function (path) {
