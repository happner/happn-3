  ...
  if (_this.happn.config.secure) {
    try {
      ...
      if (path == "/auth/request-nonce") {
        ...
        return _this.happn.services.security.createAuthenticationNonce(
          params,
          function(e, nonce) {
            if (e) return next(e);
      ...
      if (path == "/auth/login") {
        ...
        params.digest = _this.happn.services.utils.getFirstMatchingProperty(
          ["digest"],
          query
        );
      ...
      if (!session)
        return _this.__respondError(res, "invalid token format or null token");
