...
TransportService.prototype.createCertificate = function (
  keyPath,
  certPath,
  callback
) {
...
TransportService.prototype.__createHttpsServer = function (
  options,
  app,
  callback
) {
...
TransportService.prototype.createServer = function (config, app, log, callback) {
  ...
  if (['http', 'https'].indexOf(config.mode) == -1) throw new Error(
    `unknown transport mode: ${config.mode} can only be http or https`
  );
  ...
  if (!keyFileExists && !certFileExists) {
    log.warn(
      'cert file ' + config.certPath + ' is missing, trying to generate...'
    );
  }
...
TransportService.prototype.listen = function (host, port, options, callback) {
  ...
  if (!this.happn.__initialized)
    return callback(new Error('main happn service not initialized'));
...
TransportService.prototype.__tryListen = function (options) {
  ...
  if (!options.portAvailablePingInterval)
    options.portAvailablePingInterval = 1000;
  if (!options.portAvailablePingTimeout)
    options.portAvailablePingTimeout = 10000; //10 seconds

  var waitingForPortMessageInterval = setInterval(()=>{
    this.happn.log.info('port number ' + options.port + ' held by another process, retrying connection attempt...');
  }, 1000);
  ...
  tcpPortUsed
    ...
    .then(
      ...
      e => {
        this.happn.log.error('port not available: ' + e.message);
        clearInterval(waitingForPortMessageInterval);
        this.happn.__done(e);
      }
    );
