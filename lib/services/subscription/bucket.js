...
function Bucket(options) {
  ...
  if (options.action == '*') options.action = 'ALL'; //just redoing in case we had a */@blah
...
Bucket.prototype.initialize = function (callback) {
  ...
  Object.defineProperty(this, '__subscriptions', {
    value: new SortedObjectArray('segment')
  });
...
Bucket.prototype.__searchSubscriptions = function (segment, options) {
  ...
  var matchesCriteria = function (subscription) {
    ...
    if (this.options.preciseMatch) return subscription.segment == this.segment && subscription.fullPath == this.options.fullPath;
...
Bucket.prototype.getSubscriptions = function (path, options, callback) {
  ...
  segments.forEach(function (segment) {
    ...
    _this.__searchSubscriptions(segment, options).forEach(function (recipient) {
...
Bucket.prototype.__removeExplicitSubscription = function (path, sessionId, options, callback) {
  ...
  if (options.refCount >= subscription.refCount) {
    this.__explicit_subscriptions.remove(pathAndSession);
    this.__cache.reset();
    return callback();
  }

  if (typeof options.listenerId == 'undefined') {
    this.__explicit_subscriptions.remove(pathAndSession);
    this.__cache.reset();
    return callback();
  }
  ...
  delete subscription.subscriptionData[options.listenerId];
