...
PathTree.prototype._removeSubscriptionData = function(path, id, options, originalPath){
  ...
  if (subscription.refCount == 0) {
    this.__subscriptions.array.splice(subscriptionDataIndex, 1);
  }
};
...
PathTree.prototype._searchSubscriptions = function (originalPath, id) {
  ...
  if (id) {
    ...
    if (_this.branch == '*'){
      ...
      subscriptions = _this.__subscriptions.array.filter(function(subscription){
        return (subscription.id == id);
      });
    } else subscriptions = _this.__subscriptions.array.filter(function(subscription){
      return (_this.wildcardMatch(subscription.fullPath, originalPath) && subscription.id == id);
    });
    ...
    if (_this.branch == '*'){
      ...
      return (_this.wildcardMatch(subscription.fullPath, originalPath));
    });
  }
