...
Bucket.prototype.initialize = function (callback) {
  ...
  Object.defineProperty(this, '__wildcard_subscribers', {value: new SortedObjectArray('segment')});

  Object.defineProperty(this, '__explicit_subscribers', {value: new SortedObjectArray('fullPath')});

  var TrieSearch = require('trie-search');

  Object.defineProperty(this, '__prefix_subscribers', {value: new TrieSearch('prefix')});

  Object.defineProperty(this, '__prefix_subscribers_data', {value: new SortedObjectArray('fullPath')});

  Object.defineProperty(this, '__prefix_subscribers_values', {value: []});

  console.log('THIS IS A TEST BUCKET:::');
...
Bucket.prototype.getSubscriptions = function (path, options, callback) {
  ...
  if (!wildcard){
    ...
    var explicitSubscriberIndex = _this.__explicit_subscribers.search(path);

    if (explicitSubscriberIndex > -1) recipients.push({subscription: _this.__explicit_subscribers.array[explicitSubscriberIndex], index: -2});
  }
  ...
  var prefixMatches = _this.__prefix_subscribers.get(prefix);

  //console.log('searched:::', prefixMatches.length);

  if (prefixMatches.length == 0) return this.__returnSubscriptions(path, recipients, cache_key, callback)

  prefixMatches.forEach(function(subscription, index){

    if (options.preciseMatch) {
      if (subscription.fullPath == path) recipients.push({subscription: subscription, index: index});
    } else {

      //console.log('matching ' + subscription.fullPath + ' to ' + path);
...
Bucket.prototype.addSubscription = function (path, sessionId, data, callback) {
  ...
  var subscription = _this.__prefix_subscribers.get(path)[0];
  ...
  if (!subscription){
    ...
    _this.__prefix_subscribers.add(subscription);
...
Bucket.prototype.removeAllSubscriptions = function (sessionId, callback) {
  ...
  if (!sessionId) {
    ...
    this.__prefix_subscribers_data.array.forEach(function (subscription) {
  ...
  this.getSubscriptions(path, {preciseMatch: true}, function (e, subscriptions) {
    ...
    subscriptions.forEach(function (recipient) {
      ...
      if (recipient.subscription.sessionId == sessionId) {
        ...
        _this.__prefix_subscribers_data.array[recipient.index].refCount -= options.refCount;

        //refCount is not the same as listenerId
        delete recipient.subscription.subscriptionData[options.listenerId];

        if (_this.__prefix_subscribers_data.array[recipient.index].refCount <= 0) removeIndexes.push(recipient.index);
      }
    });

    removeIndexes.forEach(function (removeIndex) {
      _this.__prefix_subscribers_data.remove(removeIndex);
    });
