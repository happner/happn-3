...
SubscriptionService.prototype.__processUnnestedSubscrpitions = __processUnnestedSubscrpitions;
    ...
    async.eachSeries(
      ...
      (effectedSession, sessionCB) => {
        ...
        return this.config.allowNestedPermissions
          ...
          : this.__processUnnestedSubscrpitions(effectedSession, sessionCB);
      },
      e => {
...
function filterRecipients(message, recipients) {
  ...
  // recipients with options.wild = true are just place-holders in cse of csecurity directory changes
  let prohibited;
  ...
  recipients = recipients.filter(rec => {
    ...
    if (
      ...
      sessionList.wild.some(
        path => rec.data.searchPath.startsWith(path) || message.request.path.startsWith(path)
      )
    )
      return false;
    return true;
  ...
  let removeProhibitions = prohibitedSubs.filter(
    sub =>
      !(
        ...
        prohibitedPaths.wild.some(path => sub.data.searchPath.startsWith(path))
      )
  );
...
function __removeInvalidSubscriptions(effectedSession, allowedSubs, allowedPaths) {
  let noLongerValidSubs = allowedSubs.filter(
    sub =>
      ...
      !allowedPaths.wild.some(wildPath => sub.data.searchPath.startsWith(wildPath))
  );
  noLongerValidSubs.forEach(sub => {
    this.removeListenerExtended(
      { data: { ref: sub.data.ref, options: { prohibited: { $ne: true } } } },
      effectedSession.id,
      sub.data.action,
      sub.data.searchPath
    );
...
function __removeExplicitlyRevokedSubscriptions(effectedSession, allowedSubs, prohibitedPaths) {
  ...
  revoked.forEach(sub => {
    this.removeListener(sub.data.ref, effectedSession.id, sub.data.action, sub.data.searchPath);
  });
}
...
function __addNewProhibitions(effectedSession, prohibitedSubs, prohibitedPaths, wildSubPaths) {
  ...
  let newProhibitions = prohibitedPaths.explicit.filter(
    prohibitPath =>
      ...
      !wildProhibitedSubsPaths.some(path => prohibitPath.startsWith(path))
  );
...
function __processUnnestedSubscrpitions(effectedSession, sessionCB) {
  try {
