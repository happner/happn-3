...
function removeListener(referenceId, sessionId, action, path) {
  ...
  if (path === '*')
    return this.subscriptions.unsubscribeAll(sessionId, unsubOpts);

  return this.subscriptions.unsubscribe(sessionId, this.getSubscriptionPath(action, path), unsubOpts);
}
