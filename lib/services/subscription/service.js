...
function processMultiSubscribe(message, callback) {
  ...
  message.request.prohibited.forEach(path => {
    console.log("CALLING")
    this.removeListener(reference, message.session.id, message.request.pathData.action, path);
  });

  return this.__doSubscriptionCallback(message, reference, callback, errors.length? errors: null);
}
...
function __doSubscriptionCallback(message, reference, callback, errors) {
  ...
  this.dataService.get(
    ...
    (e, initialItems) => {
      ...
      return callback(errors.length && errors, message);
    }
  );
}
...
function removeListener(referenceId, sessionId, action, path) {
  ...
  let a = this.subscriptions.unsubscribe(
    sessionId,
    this.getSubscriptionPath(action, path),
    unsubOpts
  );
  console.log("UNSUBBED", a)
  return a;
}
