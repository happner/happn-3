...
SubscriptionService.prototype.processSubscribe = Promise.promisify(function(message, callback){
  ...
  try{
    ...
    _this.addListener(message.request.path, message.session.id, {options:message.request.options, info:message.session.info}, function(e){
...
SubscriptionService.prototype.clearSubscriptions = function(sessionId, callback){
  ...
  this.__buckets.forEach(function(bucket){
    bucket.__subscribers.array.forEach(function(subscription, index){
      if (subscription.sessionId == sessionId) bucket.__subscribers.array.splice(index, 1);
    });
  });
