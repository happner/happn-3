  ...
  try {
    ...
    this.securityService.checkpoint.listRelevantPermissions(
      ...
      (e, relevantPaths) => {
        ...
        let nestedWildSubs = allowedSubs.filter(sub => sub.data.path.endsWith('/**'));
        ...
        this.__addNewProhibitions(effectedSession, prohibitedPaths, wildSubPaths, prohibitedSubs);
    ...
    sub =>
      !(
        ...
        prohibitedPaths.wild.some(
          path => sub.data.searchPath.startsWith(path)
        )
      )
  );
...
function __addNewProhibitions(effectedSession, prohibitedPaths, wildSubPaths, prohibitedSubs) {
  let wildProhibitedSubsPaths = prohibitedSubs.filter(sub => sub.data.searchPath.endsWith('*')).map(sub => sub.data.searchPath.replace(/\*+$/, ''))
...
function __addNewSubscriptions(effectedSession, allowedPaths, prohibitedPaths, allowedSubs, wildSubs, wildSubPaths) {
