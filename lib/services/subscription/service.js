...
function initialize(config, callback) {
  ...
  try {
    ...
    if (!config.subscriptionTree.defaultVariableDepth) config.subscriptionTree.defaultVariableDepth = 5;
...
function processUnsubscribe(message) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      this.happn.services.error.handleSystem(new Error('unsubscribe failed', e), 'SubscriptionService', constants.ERROR_SEVERITY.HIGH, (e) => {
        reject(e);
      });
    }
  });
...
function processGetRecipients(message, callback) {
  ...
  try {
    ...
    this.happn.services.error.handleSystem(new Error('get recipients failed', e), 'SubscriptionService', constants.ERROR_SEVERITY.MEDIUM, (e) => {
      callback(e, message);
    });
  }
}
...
function __stripOutVariableDepthExceeded(dataPath, message, items){
  ...
  var depth = message.request.options.depth || this.config.subscriptionTree.defaultVariableDepth;
...
function __doSubscriptionCallback(message, reference, callback) {
  ...
  if (message.request.options && (message.request.options.initialCallback || message.request.options.initialEmit)) {
    ...
    this.dataService.get(dataPath, {
      ...
      if (dataPath.substring(dataPath.length - 3) == '/**') data.initialValues = this.__stripOutVariableDepthExceeded(dataPath, message, initialItems);
  ...
  return new Promise((resolve, reject) => {
    ...
    async.each(effectedSessions, (effectedSession, sessionCB) => {
      try{
        ...
        async.eachSeries(references, (reference, referenceCallback) => {
          this.securityService.checkpoint._authorizeUser(effectedSession, reference.data.path, 'on', (e, authorized) => {
            if (e) return referenceCallback(e);
            if (authorized) return referenceCallback();
            referenceCallback(null, this.removeListener(reference.data.ref, effectedSession.id, reference.data.action, reference.data.path));
          });
        }, sessionCB);
      }catch(e){
        sessionCB(e);
