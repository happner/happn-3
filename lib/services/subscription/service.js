...
SubscriptionService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    _this.subscriptions = {
      'get':Wildpare.create({mode:config.mode}),
...
SubscriptionService.prototype.processUnsubscribe = function (message, callback) {
  ...
  try {
    ...
    _this.removeListener(message.session.id, message.request.pathData.action, message.request.key, function (e) {
...
SubscriptionService.prototype.getRecipients = function (message, callback) {
  ...
  _this.subscriptions[message.request.action]
...
SubscriptionService.prototype.removeListener = function (sessionId, action, path, callback) {


  if (action == '*') return this.clearSessionSubscriptions(sessionId, callback);

  this.subscriptions[action.toLowerCase()].removeAsync({path:path, key:sessionId})
...
SubscriptionService.prototype.clearSessionSubscriptions = function (sessionId, callback) {
  ...
  _this.subscriptions['all'].removeAsync({key:sessionId})
    .then(function(references){
      removedReferences.concat(references);
      return _this.subscriptions['get'].removeAsync({key:sessionId})
    })
    .then(function(references){
      removedReferences.concat(references);
