...
SubscriptionService.prototype.processSubscribe = Promise.promisify(
  processSubscribe
);
SubscriptionService.prototype.processUnsubscribe = processUnsubscribe;
SubscriptionService.prototype.processGetRecipients = Promise.promisify(
  processGetRecipients
);
...
function initialize(config, callback) {
  try {
    ...
    if (!config.subscriptionTree.searchCache)
      //config.subscriptionTree.searchCache || 2500;
      config.subscriptionTree.searchCache = 2500; //size of LRU cache used to store search results, default 5000
    if (!config.subscriptionTree.permutationCache)
      config.subscriptionTree.permutationCache = 2500; //size of LRU cache to store path permutations, default 5000
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      this.happn.services.error.handleSystem(
        new Error('unsubscribe failed', e),
        'SubscriptionService',
...
function processGetRecipients(message, callback) {
  try {
    ...
    this.happn.services.error.handleSystem(
      new Error('get recipients failed', e),
      'SubscriptionService',
      constants.ERROR_SEVERITY.MEDIUM,
      e => {
        callback(e, message);
...
function getRecipients(message) {
  ...
  if (message.request.options && message.request.options.targetClients)
    return this.__filterTargetRecipients(
      message.request.options.targetClients,
      recipients
    );
...
function __stripOutVariableDepthExceeded(dataPath, message, items) {
  ...
  return items.filter(function(item) {
    let val = item._meta.path.substring(dataPath.length - 2).split('/');
    let valLength = val.length <= depth ? item : null;
...
function __doSubscriptionCallback(message, reference, callback) {
  ...
  if (
    ...
    (message.request.options.initialCallback ||
      message.request.options.initialEmit)
  ) {
    this.dataService.get(
      ...
      (e, initialItems) => {
        ...
        if (dataPath.substring(dataPath.length - 3) == '/**')
          data.initialValues = this.__stripOutVariableDepthExceeded(
            dataPath,
            message,
            initialItems
          );
...
function removeListener(referenceId, sessionId, action, path) {
  ...
  if (path == '*')
    return this.subscriptions.unsubscribeAll(
      this.getSubscriptionPath(action, path),
      unsubOpts
    );
