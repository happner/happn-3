var async = require('async'), Promise = require('bluebird');
...
ServiceManager.prototype.initialize = function (config, happn, callback) {
  ...
  var initializeServices = function (e, serviceNames) {
    ...
    async.eachSeries(serviceNames, function (serviceName, serviceInstanceCB) {

      var serviceInstance = _this.happn.services[serviceName];

      if (typeof serviceInstance.initialize === 'function')
        return serviceInstance.initialize(serviceInstance.__happnerSettings.config, serviceInstanceCB);
  ...
  async.eachSeries(systemServices,
    loadService,
    function (e) {

      if (e) return callback(e);

      async.eachSeries(appServices,
        loadService,
        function (e) {
