...
function __updateSession(sessionId, updated) {
  ...
  //this is so that the overridden heartbeat function in the primus wrapper can check for legacy clients
  this.__sessions[sessionId].client.happnProtocol = this.__sessions[sessionId].data.protocol;
...
function handleMessage(message, client) {
  ...
  //discard if the session no longer exists
  ...
  if (message.indexOf && message.indexOf('primus::ping') == 0){
    ...
    return this.__discardMessage(message);
  }
...
function onConnect(client) {
  ...
  //so that legacy heart beats can be handled in the primus wrapper
...
function onDisconnect(client, options) {
  ...
  _this.finalizeDisconnect(client, function(e){
    if (e) _this.log.error('client disconnect error, for session id: ' + client.sessionId, e);
  });
}
