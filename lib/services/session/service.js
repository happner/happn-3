...
SessionService.prototype.__configureSession = Promise.promisify(
  __configureSession
);
SessionService.prototype.processConfigureSession = Promise.promisify(
...
function initializeCaches(callback) {
  if (!this.config.queuedDisconnectsTTL)
    this.config.queuedDisconnectsTTL = 300000; // 5 minutes
...
function __updateSession(sessionId, updated) {
  ...
  for (var propertyName in updated)
    _this.__sessions[sessionId].data[propertyName] = updated[propertyName];

  //so we can check for the latest protocol
  _this.__sessions[sessionId].client.happnProtocol = _this.__sessions[sessionId].data.protocol;
...
function each(eachHandler, callback) {
  ...
  async.each(
    sessionKeys,
    function(sessionKey, sessionKeyCallback) {
      eachHandler.call(
        eachHandler,
        _this.__sessions[sessionKey].data,
        sessionKeyCallback
      );
    },
    callback
...
function destroyPrimus(options, callback) {
  ...
  var shutdownTimeout = setTimeout(function() {
    _this.log.error(
      'primus destroy timed out after ' + options.timeout + ' milliseconds'
    );
...
function disconnectAllClients(callback) {
  ...
  _this.each(function(sessionData, sessionDataCallback) {
    _this.disconnectSession(sessionData.id, sessionDataCallback, {
...
function initialize(config, callback) {
  ...
  try {
    ...
    _this.primus.transform('outgoing', function(packet, next) {
...
function handleMessage(message, client) {
  ...
  if (message.action === 'configure-session')
    this.__configureSession(message, client);

  //handle legacy pings
  if (message.indexOf && message.indexOf('primus::ping') == 0){
    client.onLegacyPing(message);
  ...
  _this.happn.services.queue
    ...
    .catch(function(e) {
      _this.happn.services.error.handleSystem(
        e,
...
function finalizeDisconnect(client, callback) {
  try {
    ...
    this.happn.services.subscription.clearSessionSubscriptions(
      client.sessionId
    );
...
function disconnectClient(client, callback, message) {
  ...
  if (client.__readyState === 2)
    return this.finalizeDisconnect(client, callback);
  var _this = this;
...
function onDisconnect(client, options) {
  ...
  _this.finalizeDisconnect(client, function(e) {
    if (e)
      _this.log.error(
        'client disconnect error, for session id: ' + client.sessionId,
        e
      );
  });
}
...
function securityDirectoryChanged(whatHappnd, changedData, effectedSessions) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    async.each(
      effectedSessions,
      function(effectedSession, sessionCB) {
        try {
          var client = _this.getClient(effectedSession.id);
          if (!client) return sessionCB();
          _this.happn.services.protocol.processSystemOut(
            {
