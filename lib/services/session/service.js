...
function __startUnconfiguredSessionCleanup(cleanupConfig) {
  ...
  this.__unconfiguredSessionCleanupInterval = setInterval(this.__unconfiguredSessionCleanup(cleanupConfig), cleanupConfig.interval);
}
  ...
  this.primus.destroy(
    ...
    e => {
      ...
      if (!this.__shutdownTimeout) {
        ...
        if (!e) this.log.info(`primus shutdown ok`);
        if (e) this.log.info(`primus shutdown failed: ${e.message}`);
        callback(e);
      }
    }
  );
...
function initialize(config, callback) {
  try {
    ...
    if (config.unconfiguredSessionCleanup) {
      ...
      //the interval between checks for unconfigured sessions
      config.unconfiguredSessionCleanup.interval = config.unconfiguredSessionCleanup.interval || 60e3;
      //how long we wait to classify a session with no user data as unconfigured
      config.unconfiguredSessionCleanup.threshold = config.unconfiguredSessionCleanup.threshold || 60e3;
      //log what is happening if not explicitly set to false
      config.unconfiguredSessionCleanup.verbose = config.unconfiguredSessionCleanup.verbose == null?true:config.unconfiguredSessionCleanup.verbose;
