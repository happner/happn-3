...
function __sessionIsUnconfigured(sessionId, config){
  ...
  let connectedLongerThanThreshold = Date.now() - this.__sessions[sessionId].client.happnConnected > (config.threshold || 60e3);
...
function __startUnconfiguredSessionCleanup(config){
  ...
  this.__unconfiguredSessionCleanupInterval = setInterval(() => {
    ...
    sessionKeys.forEach((sessionId) => {
      if (this.__sessionIsUnconfigured(sessionId, config)){
        this.__configuredSessionLog(`session ${sessionId} not configured, removing`, config);
