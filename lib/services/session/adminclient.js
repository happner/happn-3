...
AdminClient.prototype.off = AdminClient.prototype.removeListener = function(
  event,
  handler
) {
...
AdminClient.prototype.write = function(message) {
  ...
  setImmediate(function() {
    if (message.__outbound) {
      //dont check cloned message as it will be stripped of __outbound if it is an array
      delete message.__outbound;
      return _this.handle_publication(message); //outbound messages are cloned already
    }
    _this.context.services.session.handleMessage(
      _this.context.services.utils.clone(message),
      _this
    );
  });
};
...
AdminClient.prototype.disconnect = function(event, data) {
  ...
  _this.context.services.session.finalizeDisconnect(_this, function(e) {
    if (e)
      _this.context.services.error.handleSystem(
        e,
        "Adminclient",
        constants.ERROR_SEVERITY.LOW
      );
...
function AdminClientWrapper() {
  ...
  this.__getConnection = function(callback) {
    ...
    client.sessionProtocol =
      "happn_" + require("../../../package.json").protocol;
  ...
  this.__doLogin = function(loginParameters, callback) {
    ...
    _this.context.services.security.adminLogin(_this.socket.sessionId, function(
      e,
      session
    ) {
      ...
      _this.context.services.session.attachSession(
        _this.socket.sessionId,
        session
      );
