...
function processAcknowledge(message) {
  ...
  return new Promise(function(resolve, reject) {
    _this.__recipientAcknowledge(message, function(e, message) {
      if (e)
        return _this.errorService.handleSystem(
...
function __emitPublicationLog(publication, e, callback) {
  ...
  return this.happn.services.queue.pushOutbound(
    publication.resultsMessage(e),
    callback
  );
}
...
function __recipientAcknowledge(message, callback) {
  ...
  if (_this.__unacknowledgedPublications[message.request.data])
    _this.__unacknowledgedPublications[message.request.data].acknowledge(
...
function processPublish(message) {
  ...
  return new Promise(function(resolve, reject) {
    var publication = Publication.create(
      message,
      _this.config.publicationOptions
    );
    ...
    if (publication.options.consistency == CONSISTENCY.TRANSACTIONAL)
      return _this.happn.services.queue.pushPublication(publication, function(
        e,
        publication
      ) {
    ...
    if (
      publication.options.consistency == CONSISTENCY.QUEUED ||
      publication.options.consistency == CONSISTENCY.DEFERRED
    ) {
