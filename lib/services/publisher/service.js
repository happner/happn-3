...
PublisherService.prototype.processAcknowledge = function(message, callback){
  ...
  _this.__recipientAcknowledge(message, callback);
...
PublisherService.prototype.__emitPublicationLog = function(publication){

};

PublisherService.prototype.publicationAcknowledgeTimedOut = function(publication){

  var _this = this;

  delete _this.__unacknowledgedPublications[publication.id];

  _this.__emitPublicationLog(publication);
};
...
PublisherService.prototype.__recipientAcknowledge = function(message, callback){
  ...
  console.log('doing recipient acknowledge:::', message);

  if (_this.__unacknowledgedPublications[message.request.data]){
    _this.__unacknowledgedPublications.acknowledge(message.request.sessionId);
...
PublisherService.prototype.publishMessage = function (message, callback) {
  ...
  try{
    ...
    if (publication.options.consistency == configurator.CONSISTENCY.ACKNOWLEDGED){

      _this.__unacknowledgedPublications[publication.id] = publication;
      publication.onAcknowledgeTimedOut(_this.publicationAcknowledgeTimedOut.bind(_this))
    }
