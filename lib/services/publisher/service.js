...
function processPublish (message, callback) {
  ...
  publication.publish((e) => {
    ...
    return this.happn.services.protocol.processMessageOut(publication.resultsMessage(e), (e) => {
      if (e) this.errorService.handleSystem(new Error('acknowledged or deferred publish results feedback failed', e), 'PublisherService', constants.ERROR_SEVERITY.MEDIUM);
    });
  });
}
