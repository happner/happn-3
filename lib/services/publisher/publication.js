...
function Publication(message, options) {
  ...
  configurator.setProperty(this, 'duplicate_channels', {});

  this.configurePayload(message);

  this.configureMergePayload(message);
}
...
Publication.prototype.configureMergePayload = function (message) {

  var action = message.request.action.toUpperCase();

  var channel = ['/', action, '@', message.request.path].join();
  ...
  configurator.setProperty(this, 'mergePayload', JSON.stringify(payload));
};
