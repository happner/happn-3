...
function Publication(message, options) {
  ...
  if (this.options.consistency == this.CONSISTENCY.ACKNOWLEDGED) {
    ...
    console.log('SETUP ACK CONSISTENCY:::');
  }
...
Publication.prototype.acknowledge = function (sessionId) {
  ...
  console.log('doing acknowledge:::', sessionId, _this.unacknowledged_recipients);
...
Publication.prototype.publish = function (queue, callback) {
  ...
  async.each(_this.recipients, function (recipient, recipientCB) {
    ...
    if (_this.options.consistency == configurator.CONSISTENCY.ACKNOWLEDGED) return _this.__waitForAck(callback);
