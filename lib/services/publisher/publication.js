...
function Publication(message, options) {
  ...
  this.configurator = require('./config');

  this.configurator.setProperty(this, 'message', message);

  this.configurator.setProperty(this, 'options', message.request.options || {});
  ...
  this.configurator.setProperty(this, 'publication_options', options);
  ...
  if (this.options.consistency == CONSISTENCY.ACKNOWLEDGED) {
    ...
    this.configurator.setProperty(this, 'unacknowledged_recipients', []);
    this.options.publishResults = true;
    this.result.acknowledged = 0;
  }

  this.configurator.setProperty(this, 'id', message.session.id + '-' + message.request.eventId);
  this.configurator.setProperty(this, 'origin', message.session);
  this.configurator.setProperty(this, 'recipients', message.recipients);

  this.configurator.setProperty(this, 'duplicate_channels', {});

  this.__configurePayload(message, utils.clone(message.response));

  this.__configurePayload(message, {
    data: utils.clone(message.request.data),
    _meta: utils.clone(message.response._meta)
  }, 'mergePayload');
}
...
Publication.prototype.__configurePayload = function(message, payload, payloadType){

  if (payloadType == null) payloadType = 'payload';
  ...
  this.configurator.setProperty(this, payloadType, JSON.stringify(payload));
};
...
Publication.prototype.publish = function (queue, callback) {
  ...
  var _this = this;
  var services = queue.happn.services;
  ...
  async.each(_this.recipients, function (recipient, recipientCB) {
    ...
    if (recipient.data.options && recipient.data.options.merge == true)
      message = JSON.parse(_this.mergePayload);
    else message = JSON.parse(_this.payload);
