...
function Publication(message, options, happn) {
  ...
  else this.recipients = this.happn.services.subscription
      .getFilteredRecipients(message); 
...
Publication.prototype.publish = function(callback) {  
  if (this.wildcardRecipients && Array.isaArray(this.wildcardRecipients) &&this.wildcardRecipients.length)
  this.publishWildMessages();
  setImmediate(() => {
    async.eachLimit(
      this.recipients,
      100,
...
// Publication.prototype.publishWildMessages() {
//   let wild = this.wildcardRecipients
