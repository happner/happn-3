...
function Publication(message, options) {
  ...
  this.__configurePayload(message, message.response.data, message.response._meta);
...
Publication.prototype.publish = function (queue, callback) {
  ...
  async.each(_this.recipients, function (recipient, recipientCB) {
    ...
    if (recipient.data.options && recipient.data.options.merge == true) message = JSON.parse(_this.mergePayload);
