...
function Publication(message, options, happn) {
  ...
  let prohibited;
  [prohibited, this.recipients] = _.partition(this.recipients, ['data.options.prohibited', true]);
  this.prohibitions = {};
  if (prohibited.length) {
    this.prohibitions = prohibited
      .map(rec => rec.data)
    ...
    this.recipients = this.recipients.filter(rec => {
      let sessionList = this.prohibitions[rec.data.session.id];
