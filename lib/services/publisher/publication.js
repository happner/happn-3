...
function Publication(message, options, happn) {
  ...
  if (message.recipients) this.recipients = message.recipients; //we are targeting specific recipients for this message
  else this.recipients = this.happn.services.subscription.getRecipients(message);
...
Publication.prototype.__configurePayload = function(message, data, payloadType) {
  ...
  if (this.origin && this.origin.user) payload._meta.eventOrigin = {username: this.origin.user.username, id:this.origin.id};
...
Publication.prototype.getRecipientMessage = function(recipient) {
  ...
  var recipientMessage, channel = '/' + recipient.data.action + '@' + recipient.data.path;
  ...
  if (recipient.data.options && recipient.data.options.merge) {
    ...
    if (!this.mergePayload) this.__configurePayload(this.message, this.message.request.data, 'mergePayload');
    recipientMessage = this.mergePayload;
  } else
    recipientMessage = this.payload;

  recipientMessage._meta = Object.assign({}, recipientMessage._meta);

  recipientMessage._meta.channel = channel;
  if (recipient.data.options && recipient.data.options.depth > 0) recipientMessage._meta.depth = recipient.data.options.depth;
...
Publication.prototype.publish = function(callback) {
  ...
  setImmediate(() => {
    ...
    async.eachLimit(this.recipients, 100, (recipient, recipientCB) => {
