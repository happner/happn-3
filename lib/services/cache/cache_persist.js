...
function set(key, data, opts, callback) {
  ...
  if (opts.ttl > 0) {
    ...
    if (preExisting != null && preExisting != undefined) this.clearTimeout(key);
...
function get(key, opts, callback) {
  ...
  this.__getData(key, (e, item) => {
    ...
    if (item) {
      return this.set(key, item.data.data, {
        ttl: item.data.ttl,
        noPersist: true
      }, (e) => {
    ...
    if (opts.retrieveMethod){
      return opts.retrieveMethod.call(opts.retrieveMethod, (e, result) => {
        ...
        if (result == null || result == undefined) return this.__tryCallback(callback, null, null);
