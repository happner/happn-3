...
function set(key, data, opts, callback) {
  ...
  var cacheItem = {
    data: _this.utilities.clone(data),
    key: key
  };
  ...
  if (opts.ttl > 0) {
    ...
    if (preExisting != null && preExisting != undefined) _this.clearTimeout(preExisting);
...
function get(key, opts, callback) {
  ...
  if (_this.__cache[key]) return _this.__tryCallback(callback, _this.__cache[key].data, null, true);

  if (_this.__synced) return _this.__tryCallback(callback, null, null); //we are in sync with the db, null value
