...
function PersistedCache(opts) {
  if (!opts.dataStore)
    throw new Error("no dataStore defined for a persisted cache");
...
function set(key, data, opts, callback) {
  ...
  if (opts.ttl > 0) {
    ...
    if (preExisting != null && preExisting != undefined)
      _this.clearTimeout(preExisting);
...
function get(key, opts, callback) {
  ...
  if (cached != null)
    return this.__tryCallback(callback, cached.data, null, true);
...
function appendTimeout(data, ttl) {
  ...
  _this.__timeouts[data.key] = lt.setTimeout(function() {
    ...
    _this.remove(data.key, function(e) {
      if (e)
        _this.__emit("error", new Error("failed to remove timed out item"));
...
function clearTimeout(data) {
  if (this.__timeouts[data.key] !== undefined)
    lt.clearTimeout(this.__timeouts[data.key]);
}
