...
function initialize(config, callback) {
  try {
    ...
    this.config = config;
    this.__caches = {};
    this.__defaultCache = this.new(
      config.defaultCacheName,
      config.defaultCacheOpts
    );
...
function clear(cache, callback) {
  ...
  if (!cache || cache == _this.config.defaultCacheName)
    cacheToClear = _this.__defaultCache;
...
function filterCacheItems(filter, items) {
  if (!filter)
    return callback(this.happn.services.error.SystemError("filter is missing"));
...
function filterCache(filter, cache, callback) {
  ...
  if (!filter)
    return callback(
      _this.happn.services.error.SystemError("filter is missing")
    );
