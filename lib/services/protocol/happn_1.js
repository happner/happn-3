...
ProtocolHappn.prototype.__transformResponse = function (message, response, callback) {
  ...
  try {
    ...
    if (transformedMeta.action == 'describe') {
      response.protocol = message.session.protocol;
    }
    ...
    if (transformedMeta.action == 'on' && transformedMeta.status == 'ok') {
      ...
      delete transformedData.id;//backward compatible protocol 1
    ...
    if (transformedMeta.action == 'off' && transformedMeta.status == 'ok'){
      ...
      if (transformedData.removed) delete transformedData.removed;//backward compatible protocol 1
    }
...
ProtocolHappn.prototype.__transformRequest = function (message) {
  ...
  if (message.request && message.request.action == 'off') {
    if (message.request.options && message.request.options.listenerId != null) {
...
ProtocolHappn.prototype.__getResponseProtocol = function(message){

  //configure session has protocol in data
  if (message.data && message.data.protocol) {
    return message.data.protocol;
  }

  var session = this.happn.services.session.getSession(message.sessionId);

  if (session != null) return session.protocol;
...
ProtocolHappn.prototype.__createResponse = function (e, message, response, opts) {
  ...
  if (['on', 'off'].indexOf(message.action) == -1 && message.data && message.data.id) {
    delete message.data.id;//backward compatible - these aren't used in old clients
  }
  ...
  response.protocol = this.__getResponseProtocol(message);
...
ProtocolHappn.prototype.emit = Promise.promisify(function (message, session, callback) {
  ...
  try {
    ...
    if (client) {
      ...
      message.request.publication.protocol = session.protocol;
