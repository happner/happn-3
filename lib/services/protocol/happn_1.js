...
class ProtocolHappn1 extends BaseHappnProtocol {
  ...
  __initialEmit(message, data, callback) {
    ...
    async.eachSeries(data, (itemData, itemDataCallback) => {
      ...
      if (e) return this.happn.services.error.handleSystem(new Error('processInitialEmit failed', e), 'PublisherService', constants.ERROR_SEVERITY.MEDIUM, function(e) {
        callback(e, message);
      });
    ...
    if (transformedMeta.action == 'on' && transformedMeta.status == 'ok') {
      ...
      if (request.options.initialEmit && transformedData.length > 0) {
        this.__initialEmit(message, transformedData, (e) => {
          if (e) return this.happn.services.error.handleSystem(new Error('initialEmit failed', e), 'ProtocolHappn1', constants.ERROR_SEVERITY.MEDIUM);
        });
      }
    }
