...
function transformIn(message) {
  if (
    ...
    (message.raw.data && message.raw.data.encrypted)
  ) {
...
function __initialEmit(message, data, callback) {
  ...
  async.eachSeries(
    data,
    function(itemData, itemDataCallback) {
      ...
      _this.happn.services.publisher
        .processPublish(initialMessage)
    ...
    function(e) {
      if (e)
        return _this.happn.services.error.handleSystem(
...
function __pruneSubscriptionMappings(sessionId, references) {
  ...
  if (_this.__subscriptionMappingsListenerId[sessionId]) {
    ...
    references.forEach(function(reference) {
      var refCount =
        _this.__subscriptionMappingsListenerId[sessionId][reference.id];

      delete _this.__subscriptionMappingsListenerId[sessionId][reference.id];

      if (refCount)
        delete _this.__subscriptionMappingsReferenceId[sessionId][refCount];

      if (
        Object.keys(_this.__subscriptionMappingsListenerId[sessionId]).length ==
        0
      )
        delete _this.__subscriptionMappingsListenerId[sessionId];
      if (
        Object.keys(_this.__subscriptionMappingsReferenceId[sessionId])
          .length == 0
      )
        delete _this.__subscriptionMappingsReferenceId[sessionId];
    });
...
function __transformResponse(message, response) {
  ...
  if (transformedMeta.action == "on" && transformedMeta.status == "ok") {
    if (
      this.__subscriptionMappingsReferenceId[transformedMeta.sessionId] == null
    )
      this.__subscriptionMappingsReferenceId[transformedMeta.sessionId] = {};
    if (
      this.__subscriptionMappingsListenerId[transformedMeta.sessionId] == null
    )
  ...
  if (transformedMeta.action == "off" && transformedMeta.status == "ok")
    this.__pruneSubscriptionMappings(
      transformedMeta.sessionId,
      transformedData.removed
    );
...
function success(message) {
  var response = this.__createResponse(
    null,
    message.request,
    message.response,
    message.opts
...
function fail(message) {
  ...
  message.response = _this.__createResponse(
    message.error,
