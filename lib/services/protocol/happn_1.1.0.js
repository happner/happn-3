...
ProtocolHappn.prototype.transformIn = Promise.promisify(function(message, callback){
  ...
  try{
    ...
    if (message.raw.action  === 'login' && message.raw.data.encrypted) {
    ...
    if (message.request.action === 'set' && message.request.options){
      //there will be some options that will modify the data
      if (message.request.options.nullValue) message.request.data = null;//null values dont get passed across the wire
    }
