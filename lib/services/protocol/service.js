...
function processInboundStack(message, protocol, callback) {
  ...
  if (
    ...
    this.happn.services.security.getRelevantPaths(transformed, (e, relevantPaths) => {
      ...
      if (relevantPaths.length === 0) return  this.__processSinglePath(transformed, callback)
      let processResults = [];
      relevantPaths.forEach(path => {
        let clonedMsg = this.__utils.clone(transformed);
        clonedMsg.request.path = path;
        this.__processSinglePath(transformed, (e, result) => {
          if (result) processResults = [...processResults, ...result];
        });
      });
      return callback(null, processResults);
    });
  } else {
