  ...
  if (
    ...
    this.happn.services.security.getRelevantPaths(transformed, (e, relevantPaths) => {
      ...
      if (transformed.request.action === 'on') {
        ...
        return this.happn.services.subscription.processMultiSubscribe(transformed, function(
          e,
          result
        ) {
          if (e) return callback(e);
...
function __getProtocol(message) {
  const protocol = this.config.protocols[message.session.protocol] || this.config.protocols.happn_1;
  if (protocol != null) {
    return protocol;
  }
  if (message.session.protocol.indexOf('happn') === 0) {
    return this.config.protocols.happn_1;
  }
  throw new Error(`could not find protocol: ${message.session.protocol}`);
}
