...
function __cleanseRequestForLogs(request){
  if (request && request.data){
    return JSON.stringify(
      { username:request.data.username, info:request.data.info, protocol:request.data.protocol, options:request.options }
    );
  }
...
function __handleProtocolError(protocol, message, e, callback){
  ...
  if (e.toString().indexOf('AccessDenied: session inactivity threshold reached') == 0){
    logMessage = 'AccessDenied: session inactivity threshold reached, request: ' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }

  if (e.toString().indexOf('AccessDenied: use of _ADMIN credentials over the network is disabled') == 0){
    logMessage = 'AccessDenied: use of _ADMIN credentials over the network is disabled, request: ' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }

  if (message.session && e.toString().indexOf(`AccessDenied: session with id`) == 0 && e.toString().indexOf(`has been revoked`) > -1){
    logMessage = e.toString() + ', request:' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }
