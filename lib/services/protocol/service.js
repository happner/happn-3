...
ProtocolService.prototype.processInboundLayers = processInboundLayers;
ProtocolService.prototype.processOutboundLayers = processOutboundLayers;
...
function processInboundStack (message, protocol) {
  ...
  if (transformed.request.action === 'disconnect') return this.happn.services.session.processDisconnect(transformed);

  if (transformed.request.action === 'ack') return this.happn.services.layer.publisher.processAcknowledge(transformed);

  if (transformed.request.action === 'revoke-session') return this.happn.services.layer.session.processRevokeSession(transformed);
};
...
function processInboundLayers(message){
  var _this = this;
  return new Promise(function(resolve, reject){
    async.waterfall(_this.config.inboundLayers, function(e, result){
...
function processOutboundLayers(message){
  var _this = this;
  return new Promise(function(resolve, reject){
    async.waterfall(_this.config.outboundLayers, function(e, result){
      if (e) return reject(e);
      resolve(result);
    });
  });
...
function processMessageInLayers (message, callback) {
  ...
  var transformed;
  try{
    transformed = protocol.transformIn(layerProcessed);
  }catch(e){
    message.error = e.cause ? e.cause : e;
    callback(null, protocol.fail(message));
  }

  _this.processInboundLayers(transformed)
  .then(function (layerProcessed) {
    return _this.processInboundStack(layerProcessed);
  })
  .then(function (processed) {
    callback(null, protocol.success(processed));
  })
...
function processMessageOutLayers (message, callback) {
  ...
  try{
    ...
    _this.processOutboundLayers(protocol.transformOut(message))
