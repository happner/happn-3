...
function processInboundStack(message, protocol, callback) {
  ...
  this.happn.services.security.processAuthorize(transformed, (e, authorized) => {
    ...
    if (authorized.request.action === 'revoke-session')
      return this.happn.services.session.processRevokeSession(authorized, function(e, result) {
        if (e) return callback(e);
        callback(null, result);
      });
  });
...
function __cleanseRequestForLogs(request) {
...
function __handleProtocolError(protocol, message, e, callback) {
  ...
  var logMessage;
  var cleansedRequest = this.__cleanseRequestForLogs(message.request);
  ...
  if (e.toString() === 'AccessDenied: unauthorized' && message.request && message.request.path) {
    ...
    if (e.reason) {
      ...
      if (e.reason.indexOf('session inactivity threshold reached') === 0) {
        logMessage = 'AccessDenied: session inactivity threshold reached, request: ' + cleansedRequest;
        this.happn.services.session.disconnectSession(message.session.id, (e) => {
          if (e) return this.happn.services.log.warn(`unable to disconnect session due to inactivity-threshold: ${e.message}`, '', 'SessionService');
  ...
  if (e.toString().indexOf('AccessDenied: Invalid credentials') === 0) {
    logMessage = `${e.toString()}, request: ${cleansedRequest}`;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }

  if (e.toString() === 'AccessDenied: Account locked out') {
    logMessage = 'AccessDenied: Account locked out, request: ' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }

  if (
    e.toString().indexOf('AccessDenied: use of _ADMIN credentials over the network is disabled') ===
    0
  ) {
    logMessage =
      'AccessDenied: use of _ADMIN credentials over the network is disabled, request: ' +
      cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }
  ...
  if (
    message.session &&
    e.toString().indexOf(`AccessDenied: session with id`) === 0 &&
    e.toString().indexOf(`has been revoked`) > -1
  ) {
    logMessage = e.toString() + ', request:' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }

  if (e.toString().indexOf(`AccessDenied: expired session token`) === 0) {
    logMessage = e.toString() + ', request:' + cleansedRequest;
    return this.happn.services.log.warn(logMessage, '', 'SecurityService');
  }
