  ...
  if (!this.__stackCache[stackKey]){
    ...
    if (['set', 'remove'].indexOf(transformed.request.action) >= 0){
      ...
      if (!transformed.request.options || !transformed.request.options.noPublish) {
        ...
        var consistency = transformed.request.options ? transformed.request.options.consistency : 2;//level 1, transactional by default

        if (consistency == 0) cachedStack.push(this.happn.services.layer.publisher.processOptimistic);//optimistic, fire and forget - response comes back before the publication is queued

        if (consistency == 1) cachedStack.push(this.happn.services.layer.publisher.processQueued);//queued, response comes back after the publication is queued

        if (consistency == 2) cachedStack.push(this.happn.services.layer.publisher.processTransactional);//transactional, response comes back after the publication has reached all recipients

        if (consistency == 3) cachedStack.push(this.happn.services.layer.publisher.processAcknowledged);//acknowledged, response comes back after each recipient has acknowledged receiving the publication
