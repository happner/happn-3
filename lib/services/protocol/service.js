...
function processMessageIn (message, callback) {
  ...
  if (!protocol) {
    ...
    if (message.session.protocol.indexOf('happn_1') == 0) protocol = this.config.protocols['happn_1'];
    else return callback(this.happn.services.error.SystemError('unknown inbound protocol: ' + message.session.protocol, 'protocol'));
  }
