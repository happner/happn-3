...
function processSystem(message, respond) {
  if (!this.config.protocols[message.session.protocol])
    return respond(
  ...
  respond(
    null,
    this.config.protocols[message.session.protocol].transformSystem(message)
  );
}
...
function processSystemOut(message, callback) {
  try {
    var systemEvent = this.config.protocols[
      message.session.protocol
    ].transformSystem(message);
    if (systemEvent.__suppress) return callback();
    this.config.protocols[message.session.protocol].emit(
      systemEvent,
      message.session
    );
    callback();
...
function processInboundStack(message, protocol) {
  ...
  if (['set', 'remove'].indexOf(transformed.request.action) > -1) {
    return _this.happn.services.security
      .processAuthorize(transformed)
      .then(function(authorized) {
        if (transformed.request.action == 'set')
          return _this.happn.services.data.processStore(authorized);
      ...
      .then(function(publication) {
        if (
          ...
          publication.request.options.noPublish
        )
  ...
  if (transformed.request.action == 'on') {
    return _this.happn.services.security
      .processAuthorize(transformed)
      .then(function(authorized) {
        return _this.happn.services.subscription.processSubscribe(
          _this.happn.services.subscription.prepareSubscribeMessage(authorized)
        );
      });
  }

  if (transformed.request.action == 'off')
    return _this.happn.services.subscription.processUnsubscribe(
      _this.happn.services.subscription.prepareSubscribeMessage(transformed)
    );

  if (transformed.request.action === 'request-nonce')
    return this.happn.services.security.processNonceRequest(transformed);

  if (transformed.request.action === 'describe')
    return this.happn.services.system.processMessage(transformed);

  if (transformed.request.action === 'configure-session')
    return this.happn.services.session.processConfigureSession(transformed);

  if (transformed.request.action === 'login') {
    if (this.config.secure)
      return this.happn.services.security.processLogin(transformed);
    else return this.happn.services.security.processUnsecureLogin(transformed);
  }

  if (transformed.request.action === 'ack')
    return this.happn.services.publisher.processAcknowledge(transformed);

  if (transformed.request.action === 'revoke-session')
    return this.happn.services.session.processRevokeSession(transformed);
}
...
function __handleProtocolError(protocol, message, e, callback) {
  ...
  if (
    e.toString() == 'AccessDenied: unauthorized' &&
    message.request &&
    message.request.path
  ) {
    var logMessage =
      'AccessDenied: unauthorized, path: ' + message.request.path;
...
function processMessageInLayers(message, callback) {
  ...
  _this
    .processLayers(message, _this.config.inboundLayers)
...
function initialize(config, callback) {
  ...
  if (this.config.outboundLayers)
    this.processMessageOut = this.processMessageOutLayers;
  if (this.config.inboundLayers)
    this.processMessageIn = this.processMessageInLayers;
