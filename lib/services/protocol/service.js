...
function __handleProtocolError(protocol, message, e, callback){
  ...
  if (e.toString() == 'AccessDenied: unauthorized' && message.request && message.request.path){
    ...
    return this.happn.services.log.warn(logMessage, '', 'ProtocolService');
  }

  if (e.toString() == 'AccessDenied: unauthorized' && message.session && message.session.user && message.session.user.username){
