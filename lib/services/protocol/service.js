...
function processInboundStack(message, protocol, callback) {
  ...
  if (
    ...
    this.happn.services.security.getRelevantPaths(transformed, (e, relevantPaths) => {
      ...
      try {
      if (transformed.request.action === 'on' && relevantPaths.allowed.some(path => transformed.request.path.startsWith(path))) {
        return this.__processSinglePath(transformed, callback);
      }
    } catch(e) {console.log(e)}
      let clonedMsg = JSON.parse(JSON.stringify(transformed));
      //clonedMsg.request.wildPath = clonedMsg.request.path;
      ...
      async.series(
        ...
        (e, msgs) => {
          ...
          aggregated.response = responses.filter(msg =>{
            console.log(msg)
          return  (!msg._meta || !msg._meta.path ||  !relevantPaths.prohibited.includes(msg._meta.path));
          })
          console.log(aggregated.response)
          if (aggregated.response.length= 1) aggregated.response = aggregated.response[0];
