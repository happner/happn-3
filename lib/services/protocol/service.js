...
function processMessageIn (message, callback) {
  ...
  .catch(function (e) {
    ...
    if (e.toString() == 'AccessDenied: unauthorized' && message.request && message.request.path){
      ...
      _this.happn.services.log.warn(logMessage, '', 'ProtocolService');

    } else _this.happn.services.error.handleSystem(e, 'ProtocolService', constants.ERROR_SEVERITY.MEDIUM);
  });
}
...
function processMessageInLayers (message, callback) {
  ...
  .catch(function (e) {
    message.error = e.cause ? e.cause : e;
    callback(null, protocol.fail(message));
    _this.happn.services.error.handleSystem(e, 'ProtocolService', constants.ERROR_SEVERITY.MEDIUM);
  });
}
