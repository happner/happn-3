...
function processInboundStack (message, protocol) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try{
      transformed = protocol.transformIn(message);
      if (transformed.__suppress) return resolve(message);
    }catch(e){
      return reject(e);
    }
    ...
    this.happn.services.security.processAuthorize(transformed, (e, authorized) => {
      ...
      if (authorized.request.action == 'set')
        return this.happn.services.data.processStore(authorized, (e, publication) => {
          if (e) return reject(e);
          if (publication.request.options && publication.request.options.noPublish) return resolve(publication);
      ...
      if (authorized.request.action === 'describe') return resolve(this.happn.services.system.processMessage(authorized));
...
function processMessageIn (message, callback) {
  ...
  this.processInboundStack(message, protocol)
  .then((processed) => {
