  ...
  if (!this.__stackCache[stackKey]){
    ...
    if (['set', 'remove'].indexOf(transformed.request.action) >= 0){
      ...
      if (!transformed.request.options || !transformed.request.options.noPublish) {
        ...
        var consistency = transformed.request.options ? transformed.request.options.consistency : 1;//level 1, transactional by default

        if (consistency == 0) cachedStack.push(this.happn.services.layer.publisher.processOptimistic);//optimistic

        if (consistency == 1) cachedStack.push(this.happn.services.layer.publisher.processTransactional);//transactional

        if (consistency == 2) cachedStack.push(this.happn.services.layer.publisher.processAcknowledged);//acknowledged
