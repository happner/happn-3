...
function processInboundStack(message, protocol, callback) {
  ...
  if (
    ...
    this.happn.services.security.getRelevantPaths(transformed, (e, relevantPaths) => {
      console.log({
        relevantPaths,
        action: transformed.request.action,
        path: transformed.request.path
      });
      if (e) return callback(e);
      if (relevantPaths.allowed.length === 0) {
        return this.__processSinglePath(transformed, callback);
      }
      console.log({ relevantPaths });
      let clonedMsg = JSON.parse(JSON.stringify(transformed));
      if (transformed.request.action === 'on') {
        let channelAction = transformed.request.path.split('@')[0];
        let channelPath = transformed.request.path
          .split('@')
          .slice(1)
          .join('@'); //Join just in case their are  @'s later in the path

        transformed.request = {...transformed.request, ...relevantPaths};

        if (relevantPaths.allowed.some(allowedPath => channelPath.startsWith(allowedPath))) {
          //only happens if we request "path/**" and have permissions on "path/*"
          return this.__processSinglePath(transformed, callback);
        }
