...
function __handleProtocolError(protocol, message, e, callback) {
  ...
  if (errorMessage.indexOf('expired session token') >= 0) {
    logMessage = 'AccessDenied: expired session token, request: ' + cleansedRequest;
    this.happn.services.session.disconnectSession(
      message.session.id,
      e => {
        if (e)
          return this.happn.services.log.warn(
            `unable to disconnect session due to expired session token: ${e.message}`,
            '',
            'SessionService'
          );
      },
  ...
  if (errorMessage.indexOf('session inactivity threshold reached') >= 0) {
