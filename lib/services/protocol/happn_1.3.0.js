...
ProtocolHappn.prototype.initialize = function(happn){

  this.happn = happn;

  var _this = this;

  happn.services.session.on('client-disconnect', function(sessionId){
...
ProtocolHappn.prototype.transformOut = Promise.promisify(function (message, callback) {
  ...
  try {
    ...
    message.request = this.__transformEmit(message.raw);
...
ProtocolHappn.prototype.__transformEmit = function(message){
    ...
    var initialMessage = {
      ...
      recipients: [{
        data: {
          ...
          session: {protocol: session.protocol, id: session.id},
...
ProtocolHappn.prototype.__transformResponse = function(message, response, callback){
  ...
  try{
    ...
    if (transformedMeta.action == 'off' && transformedMeta.status == 'ok'){
      ...
      transformedMeta.listenerId = this.__subscriptionMappingsListenerId[transformedMeta.sessionId][transformedData.id];

      delete this.__subscriptionMappingsListenerId[transformedMeta.sessionId][transformedData.id];
      delete this.__subscriptionMappingsReferenceId[transformedMeta.sessionId][transformedMeta.listenerId];

      if (Object.keys(this.__subscriptionMappingsListenerId[transformedMeta.sessionId]).length == 0) delete this.__subscriptionMappingsListenerId[transformedMeta.sessionId];
      if (Object.keys(this.__subscriptionMappingsReferenceId[transformedMeta.sessionId]).length == 0) delete this.__subscriptionMappingsReferenceId[transformedMeta.sessionId];
    }
...
ProtocolHappn.prototype.__transformRequest = function(message){
  ...
  if (message.request && message.request.action == 'off'){
    if (message.request.options.listenerId != null){
      message.request.options.referenceId = this.__subscriptionMappings[message.request.sessionId][message.request.options.listenerId];
    }
  }
