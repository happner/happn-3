...
ProtocolHappn.prototype.__encryptMessageiv = __encryptMessageiv;
...
function emit(message, session) {
  ...
  if (client) {
    ...
    if (session.isEncrypted)
      message.request.publication = {
        encrypted: this.__encryptMessageiv(
          message.request.publication,
          session.secret
        )
      };
...
function __encryptMessageiv(response, secret) {
  var iv = this.happn.services.utils.computeiv(secret);
...
function fail(message) {
  ...
  if (message.request.action != 'login' && message.session.isEncrypted) {
    ...
    message.response = {
      encrypted: _this.__encryptMessageiv(
        message.response,
        message.session.secret
      )
    };
