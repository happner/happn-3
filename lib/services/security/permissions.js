...
module.exports = class Permissions {
  ...
  async upsertMultiplePermissions(name, permissions) {
    if (!name) {
      throw new Error(`please supply a ${this.type}Name`);
    }
    const permissionsValidation = this.validatePermissions(permissions);
    if (permissionsValidation !== true) {
      throw new Error(`${this.type} permissions invalid: ` + permissionsValidation.join(','));
    }
    for (let [permissionPath, permission] of Object.entries(permissions)) {
      await this.__upsertPermissions(name, permissionPath, permission.actions);
      await this.__upsertPermissions(name, permissionPath, permission.prohibit, false);
    }
  }
