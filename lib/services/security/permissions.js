...
module.exports = class Permissions {
  ...
  attachPermissions(entity, includeProhibited = false) {
    entity.permissions = {};

    return this.listPermissions(entity.name || entity.username).then(permissions => {
      console.log({permissions})
      // const prohibitedWildcardList = permissions.reduce((object, { action, authorized, path }) => {
      //   if (!authorized && path.endsWith('/*')) {
      //     if (!(action in object)) {
      //       object[action] = [];
      //     }

      //     object[action].push(path.slice(0, -1));
      //   }
      ...
      permissions.forEach(permission => {
        ...
        if (includeProhibited && permission.authorized === false) {
          if (permission.action in prohibitedWildcardList) {
            for (const prohibited of prohibitedWildcardList[permission.action]) {
              if (permission.path.startsWith(prohibited)) return;
            }
          }
