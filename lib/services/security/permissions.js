...
module.exports = class Permissions {
  ...
  __removePermission(name, path, action, removeProhibition = false) {
    return new Promise((resolve, reject) => {
      this.dataService.get(
        [
        ...
        (e, storedData) => {
          storedData.forEach(stored => {
            if (e) return reject(e);
            if (!stored || !stored.data) resolve();
            if (removeProhibition && stored.data.authorized === true) resolve();
            if (!removeProhibition && stored.data.authorized === false) resolve();
  ...
  validatePermissions(permissions) {
    ...
    Object.keys(permissions).forEach(function(permissionPath) {
      ...
      if (!permission.actions && !permission.prohibit && !permission.remove)
        return errors.push(
          'missing allowed actions or prohibit rules, or rules to remove: ' + permissionPath
        );

      Object.keys(permission).forEach(changeType => {
        permission[changeType].forEach(action => {
