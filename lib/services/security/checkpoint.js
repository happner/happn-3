  ...
  return PermissionsTree.create(
    Object.keys(permissions).reduce((permissionSet, rawPath) => {
      ...
      if (rawPath.indexOf('{{[') > -1) {
        let start = rawPath.indexOf('{{[');
        let end = rawPath.indexOf(']}}');

        let key = rawPath.slice(start + 3, end);
        const values = _.get(identity, key);

        if (Array.isArray(values)) {
          for (const v of values) {
            templatedPath = rawPath.replace(`{{[${key}]}}`, v);
            permissionSet[templatedPath] = permissions[rawPath];
          }
          return permissionSet;
        }
      } else if (rawPath.indexOf('{{') > -1) {
