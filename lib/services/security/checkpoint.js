...
function CheckPoint(opts) {
  ...
  String.prototype.count = function(chr) {
    ...
    for (var i = first; i < this.length; ++i) if (c === this.charAt(i)) ++count;
...
CheckPoint.prototype.clearCaches = function(effectedSessions) {
  return new Promise((resolve, reject) => {
    try {
      if (this.__cache_checkpoint_permissionset)
        this.__cache_checkpoint_permissionset.clear();
      if (this.__cache_checkpoint_authorization)
...
CheckPoint.prototype.__authorizedAction = function(action, permission) {
  for (var permissableActionIndex in permission.actions) {
    if (
      permission.actions[permissableActionIndex] == action ||
      permission.actions[permissableActionIndex] == "*"
    )
      return true;
  }
...
CheckPoint.prototype.__authorized = function(permissionSet, path, action) {
  for (var permissionPath in permissionSet.explicit) {
    if (permissionPath == path) {
      if (
        this.__authorizedAction(action, permissionSet.explicit[permissionPath])
      )
        return true;
    }
  }
  ...
  for (var wildPermissionPath in permissionSet.wildcard) {
    if (
      this.securityService.happn.services.utils.wildcardMatch(
        wildPermissionPath,
        path
      )
    ) {
      if (
        this.__authorizedAction(
          action,
          permissionSet.wildcard[wildPermissionPath]
        )
      )
        return true;
    }
...
CheckPoint.prototype.__compilePermissionsTemplate = function(
  permissionPath,
  identity
) {
  var cachedTemplate = this.__cache_compiled_permissions_templates.getSync(
    permissionPath
  );

  if (!cachedTemplate) {
    cachedTemplate = handlebars.compile(permissionPath, { strict: true });
    this.__cache_compiled_permissions_templates.setSync(
      permissionPath,
      cachedTemplate,
      { clone: false }
    );
...
CheckPoint.prototype.__loadPermissionSet = function(identity, callback) {
  var permissionSet = this.__cache_checkpoint_permissionset.getSync(
  ...
  async.each(
    ...
    e => {
      ...
      this.__cache_checkpoint_permissionset.setSync(
        identity.permissionSetKey,
        permissionSet,
        { clone: false }
      );
...
CheckPoint.prototype.__constructPermissionSet = function(session, callback) {
  if (session.isToken) {
    //we are dealing with a decoded token, re-fetch the user
    ...
    this.__loadPermissionSet(session, callback);
  }
};
...
CheckPoint.prototype.initialize = function(config, securityService, callback) {
  try {
    ...
    if (!config.__cache_checkpoint_permissionset)
      config.__cache_checkpoint_permissionset =
        config.__cache_checkpoint_authorization;

    if (!config.expiry_grace) config.expiry_grace = 60; //default expiry grace of 1 minute

    if (!config.groupPermissionsPolicy)
      config.groupPermissionsPolicy = "most_restrictive";
    ...
    this.__checkpoint_usage_limit = this.cacheService.new(
...
CheckPoint.prototype.__checkInactivity = function(session, policy, callback) {
  if (
    policy.inactivity_threshold <= 0 ||
    policy.inactivity_threshold == Infinity ||
    policy.inactivity_threshold == null ||
    policy.inactivity_threshold == undefined
  )
  ...
  this.__checkpoint_inactivity_threshold.get(session.id, (e, value) => {
    ...
    if (value == null || value == undefined) {
      if (now - session.timestamp > policy.inactivity_threshold)
        return callback(null, false);
      if (now - session.timestamp < policy.inactivity_threshold) doSet();
    } else {
      if (now - value > policy.inactivity_threshold)
        return callback(null, false);
...
CheckPoint.prototype.__checkUsageLimit = function(session, policy, callback) {
  if (
    policy.usage_limit == null ||
    policy.usage_limit <= 0 ||
    policy.usage_limit == Infinity
  )
    return callback(null, true);
...
CheckPoint.prototype.__checkSessionPermissions = function(
  policy,
  path,
  action,
  session
...
CheckPoint.prototype._authorizeSession = function(
  session,
  path,
  action,
  callback
  ...
  try {
    ...
    if (!policy)
      return callback(
        null,
        false,
        "no policy for session type: " + session.type
      );
    ...
    this.__checkInactivity(session, policy, (e, ok) => {
      if (e) return callback(e);
      if (!ok)
        return callback(null, false, "session inactivity threshold reached");
      ...
      this.__checkUsageLimit(session, policy, (e, ok) => {
        ...
        if (policy.permissions) {
          ...
          var passthrough = this.__checkSessionPermissions(
            policy,
            path,
            action,
            session
...
CheckPoint.prototype._authorizeUser = function(
  session,
  path,
  action,
  callback
  ...
  var authorized = this.__cache_checkpoint_authorization.getSync(
    permissionCacheKey
  );
  ...
  if (authorized == null)
    return this.__constructPermissionSet(session, (e, permissionSet) => {
      ...
      this.__cache_checkpoint_authorization.setSync(
        permissionCacheKey,
        authorized,
        { clone: false }
      );
