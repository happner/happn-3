...
function CheckPoint(opts) {
  ...
  String.prototype.count = function(chr) {
    let count = 0, first = this.indexOf(chr);
...
CheckPoint.prototype.stop = function () {
  ...
  if (this.__cache_compiled_permissions_templates) this.__cache_compiled_permissions_templates.clear();
...
CheckPoint.prototype.__authorizedAction = function (action, permission) {
  ...
  for (let permissableAction of permission.actions) {
    if (permissableAction == action || permissableAction == '*')
      return true;
  }
...
CheckPoint.prototype.__createPermissionSet = function (permissions, identity) {
  ...
  permissionSet.wildcard = this.securityService.happn.services.utils.wildcardAggregatePermissions(permissionSet.wildcard);
...
CheckPoint.prototype.__loadPermissionSet = function (identity, callback) {
  ...
  async.each(Object.keys(identity.user.groups), (groupName, eachCB) => {
    ...
    this.__cache_checkpoint_permissionset.setSync(identity.permissionSetKey, permissionSet, {clone:false});
...
CheckPoint.prototype.__constructPermissionSet = function (session, callback) {
  ...
  this.securityService.users.getUser(session.username, (e, user) => {
    ...
    if (e) return callback(e);
    if (user == null) return callback(this.errorService.AccessDeniedError('user ' + session.username + ' has been deleted or does not exist'));
...
CheckPoint.prototype.__checkInactivity = function (session, policy, callback) {
  ...
  if (!policy.inactivity_threshold || policy.inactivity_threshold == Infinity) return callback(null, true);
...
CheckPoint.prototype.__checkUsageLimit = function (session, policy, callback) {
  ...
  this.__checkpoint_usage_limit.get(session.id, {
    default: {
      value: 0,
      ttl: ttl
    }
...
CheckPoint.prototype._authorizeSession = function (session, path, action, callback) {
  ...
  try {
    ...
    if (session.policy == null) return callback(null, false, 'no policy attached to session: ' + JSON.stringify(session));
    ...
    this.__checkInactivity(session, policy, (e, ok) => {
      ...
      this.__checkUsageLimit(session, policy, (e, ok) => {
        ...
        if (policy.permissions) {
          ...
          if (this.__checkSessionPermissions(policy, path, action, session)) return callback(null, true, null, true);
