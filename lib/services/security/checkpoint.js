...
CheckPoint.prototype.__loadPermissionSet = function (identity, callback) {
  ...
  async.each(Object.keys(identity.user.groups), function (groupName, eachCB) {
    ...
    _this.securityService.users.getGroup(groupName, {}, function (e, group) {
      ...
      if (!group) return _this.log.warn('matching group ' + groupName + ', for user permissions not found.');

      for (var permissionPath in group.permissions) permissions[permissionPath] = group.permissions[permissionPath];
...
CheckPoint.prototype.clearCaches = function () {
  ...
  return new Promise(function (resolve) {
    _this.__permissionSets = {}; //clear the permission set cache
