...
function processLogin(message, callback) {
  ...
  _this.login(credentials, sessionId, function (e, session) {
    ...
    var decoupledSession = _this.happn.services.utils.clone(_this.happn.services.session.attachSession(message.session.id, session));
...
function getEffectedSession(sessionData) {
  ...
  return {
    ...
    isToken: sessionData.isToken,
    permissionSetKey: sessionData.permissionSetKey,
    user:{
      groups:sessionData.user.groups
    }
  };
}
...
function resetSessionPermissions(whatHappnd, changedData) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (['link-group', 'unlink-group', 'delete-user', 'delete-group', 'upsert-group', 'permission-removed', 'permission-upserted'].indexOf(whatHappnd) > -1) {
        ...
        try {
          ...
          if (whatHappnd == 'unlink-group') {
            ...
            if (changedData.path.indexOf('/_SYSTEM/_SECURITY/_USER/' + sessionData.user.username + '/_USER_GROUP/') == 0) {
              ...
              var groupName = changedData.path.replace('/_SYSTEM/_SECURITY/_USER/' + sessionData.user.username + '/_USER_GROUP/', '');
...
function generateToken(session) {
  ...
  console.log('generateToken decoupledSession:::', decoupledSession.user.username);
  console.log('generateToken session:::', session.user.username);
...
function generateSession(user, sessionId, credentials, callback) {
  ...
  console.log('have session:::', JSON.stringify(session));
  ...
  console.log('profiled session:::', JSON.stringify(session.user));
...
function __checkLockedOut(username, callback) {
