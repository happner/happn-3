...
function processAuthorize(message, callback) {
  ...
  if (AUTHORIZE_ACTIONS.indexOf(message.request.action) == -1)
    return callback(null, message);

  const authPath = message.request.path.replace(/^\/(?:REMOVE|SET|ALL)@/, '');

  if (message.request.options && message.request.options.onBehalfOf && message.request.options.onBehalfOf != '_ADMIN')
  ...
  return new Promise((resolve, reject) => {
    ...
    this.dataService.get('/_SYSTEM/_SECURITY/_SETTINGS/KEYPAIR', {}, (e, response) => {
      ...
      try {
        ...
        let transformedKeyPair = this.cryptoService.serializeKeyPair(this.cryptoService.keyPairFromWIF(response.data.value));
...
function __initializeOnBehalfOfCache() {
  ...
  return new Promise((resolve) => {
    ...
    this.__cache_session_on_behalf_of = this.cacheService.new('cache_session_on_behalf_of', this.options.onBehalfOfCache);
    resolve();
  });
}
...
function __checkRevocations(sessionId, callback, notFromTokenLogin) {
  ...
  this.__cache_revoked_sessions.get(sessionId, function (e, item) {
    ...
    if (notFromTokenLogin) return callback(null, false, `session with id ${sessionId} has been revoked`);
...
function revokeSession(session, reason, callback) {
  ...
  this.__cache_revoked_sessions.set(session.tokenOriginSessionId || session.id, {
    reason: reason,
    id: session.id
  }, {
    ttl: ttl
    ...
    if (!e) this.dataChanged('session-revoked', session, `session with id ${session.id}  and origin ${session.tokenOriginSessionId?session.tokenOriginSessionId:session.id} revoked`);
    callback(e);
  });
}
...
function sessionFromRequest(req, options) {
  ...
  try {
    ...
    this.log.warn(`failed decoding session token from request, token: ${token}, error: ${e.message}`);
    return null;
  }
}
...
function restoreSession(sessionId, callback) {
  ...
  this.__cache_revoked_sessions.get(sessionId, (e, session) => {
    ...
    this.__cache_revoked_sessions.remove(sessionId, (e) => {
      if (!e) this.dataChanged('session-restored', session, `session with id ${session.id} restored`);
      callback(e);
    });
  });
}
...
function resetSessionPermissions(whatHappnd, changedData) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (['link-group', 'unlink-group', 'delete-user', 'delete-group', 'upsert-group', 'permission-removed', 'permission-upserted', 'upsert-user', 'session-revoked', 'session-restored'].indexOf(whatHappnd) == -1) return resolve();

    if (whatHappnd == 'session-revoked') {
      effectedSessions.push(getEffectedSession(changedData, true));
      if (!changedData.replicated) return resolve(effectedSessions);//means this is the originating process
      //means we are getting a replication from elsewhere in the cluster
      return this.__cache_revoked_sessions.set(changedData.tokenOriginSessionId || changedData.id, {reason: changedData.reason, id: changedData.id}, { noPersist: true, ttl:changedData.ttl }, (e) => {
        if (e) return reject(e);
    ...
    if (whatHappnd == 'session-restored') {
      ...
      return this.__cache_revoked_sessions.remove(changedData.id, { noPersist: changedData.replicated }, function(e){
        if (e) return reject(e);
        resolve(effectedSessions);
      });
    }
    ...
    try {
      ...
      return this.authorize(previousSession, null, 'login', (e, authorized, reason) => {
        ...
        this.users.getUser(previousSession.username, (e, user) => {
          ...
          return this.__loginOK(credentials, user, sessionId, callback, previousSession.tokenOriginSessionId || previousSession.id);
        });
      });
...
function authorize(session, path, action, callback) {
  ...
  this.__checkRevocations(session.tokenOriginSessionId || session.id, (e, authorized, reason) => {
...
function authorizeOnBehalfOf(session, path, action, onBehalfOf, callback) {
  ...
  if (session.user.username != '_ADMIN') return completeCall(null, false, 'session attempting to act on behalf of is not authorized');
...
function __getOnBehalfOfSession(delegatedBy, onBehalfOf, callback){
  ...
  this.users.getUser(onBehalfOf, (e, onBehalfOfUser) => {
    ...
    behalfOfSession = this.generateSession(onBehalfOfUser, null, {info:{delegatedBy:delegatedBy.user.username}});
