...
SecurityService.prototype.listActiveSessions = util.promisify(listActiveSessions);
...
SecurityService.prototype.doAudit = util.promisify(doAudit);
...
function initialize(config, callback) {
  ...
  this.__initializeGroups(config)
    ...
    .then(() => {
      return this.__initializeReplication(config);
    })
    .then(() => {
      return this.__initializeSessionTokenSecret(config);
    })
    .then(() => {
      callback();
    })
...
function resetSessionPermissions(whatHappnd, changedData) {
  return new Promise((resolve, reject) => {
    ...
    if (CONSTANTS.SECURITY_DIRECTORY_EVENTS_COLLECTION.indexOf(whatHappnd) === -1) return resolve();
    ...
    this.sessionService.each(
      (sessionData, sessionCallback) => {
        ...
        if (whatHappnd === CONSTANTS.SECURITY_DIRECTORY_EVENTS.PERMISSION_REMOVED) {
        ...
        if (whatHappnd === CONSTANTS.SECURITY_DIRECTORY_EVENTS.PERMISSION_UPSERTED) {
          if (sessionData.user.groups[changedData.groupName] != null) {
            //all we need to do, permissionSetKey remains the same (as it is the ordered list of linked groups) - all caches are cleared, but effected sessions are different
            effectedSessions.push(getEffectedSession(sessionData, true));
          }
        ...
        if (whatHappnd === CONSTANTS.SECURITY_DIRECTORY_EVENTS.LINK_GROUP) {
          if (
        ...
        if (whatHappnd === CONSTANTS.SECURITY_DIRECTORY_EVENTS.UPSERT_GROUP) {
  ...
  if (credentials.token) {
    try {
      return this.__checkRevocations(credentials.token, (e, authorized, reason) => {
        ...
        if (previousSession.type != null && this.config.lockTokenToLoginType) {
