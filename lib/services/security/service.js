...
function processLogin(message, callback) {
  ...
  if (credentials.username === '_ADMIN' && this.authProviders.happn3)
    authProvider = this.authProviders.happn3;
...
function __initializeAuthProviders(config) {
  let authProviders = config.authProviders || { happn3: 'happn3-provider' };
  let defaultAuthProvider = config.defaultAuthProvider || 'happn3';
  this.authProviders = {};
  Object.keys(authProviders).forEach(key => {
    let provider = authProviders[key];
    let internalPath = path.resolve(__dirname, `./authentication/${provider}.js`);
    this.authProviders[key] = fs.existsSync(internalPath)
      ? require(internalPath).create(this.happn, config)
      : BaseAuthProvider.create(this.happn, config, provider);
  });
  this.authProviders.default = this.authProviders[defaultAuthProvider] || this.authProviders[0]; //The or is for cases where you are not using happn3 auth provider, but have not specified another default provider
}
...
function generateSession(user, sessionId, credentials, tokenLogin, additionalInfo) {
  ...
  // if (additionalInfo) session = {...session, ...additionalInfo}
  additionalInfo = additionalInfo || {};
  if (tokenLogin) session.token = tokenLogin.token;
  else session.token = this.generateToken({ ...session, ...additionalInfo }); //Don't want additional info to stay around unencrypted (?)
...
function getRelevantPaths(message, callback) {
  const authPath = message.request.path.replace(/^\/(?:REMOVE|SET|ALL)@/, '');
  this.checkpoint.listRelevantPermissions(
    message.session,
    authPath,
    message.request.action,
    callback
