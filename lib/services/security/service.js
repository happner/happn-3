    ...
    if (credentials.token){
      ...
      try{
        ...
        return _this.authorize(previousSession, null, 'login', function(e){
          ...
          if (e) return callback(_this.happn.services.error.AccessDeniedError('invalid token'));

          _this.users.getUser(credentials.username, function (e, user) {

            if (user == null) return callback(_this.happn.services.error.AccessDeniedError('user does not exist'));
        ...
        return callback(_this.happn.services.error.AccessDeniedError('Invalid credentials: ' + e.toString()));
      }
