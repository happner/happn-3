...
SecurityService.prototype.processAuthorize = Promise.promisify(function (message, callback) {
  ...
  //on's are allowed (they start with ACTION@ anyhow)
  if (message.request.path.indexOf('/_SYSTEM/') == 0) return callback(_this.happn.services.error.AccessDeniedError('unauthorized', 'attempt to set or remove a system path starting with /_SYSTEM'));
...
SecurityService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    if (!config.securityDirectoryChangedCheckInterval) config.securityDirectoryChangedCheckInterval = 1000;

    this.config = config;

    var _this = this;

    _this.__initializeCheckPoint(config, function (e) {
          ...
          _this.__initializeProfiles(config, function (e) {
            ...
            _this.__ensureKeyPair(config, function (e) {
              ...
              _this.__ensureAdminUser(config, function (e) {
                ...
                _this.__initializeSecurityDirectyChanged(config, callback);
...
SecurityService.prototype.stop = Promise.promisify(function (options, callback) {

  if (this.securityDirectyChangedCheck) clearInterval(this.securityDirectyChangedCheck);
...
SecurityService.prototype.reconfigureSessionSecurity = function (session, callback) {

  var _this = this;

  //get the user and groups
  _this.users.getUser(session.user.username)
    .then(function (user) {

      if (user == null) {
        //no user, disconnect session
        _this.sessionService.disconnectSession(session.id);
        return callback();
      }

      return new Promise(function (resolve) {

        var permissionSetKey = _this.generatePermissionSetKey(user);

        //set new permission set key and user
        session.user = user;
        session.permissionSetKey = permissionSetKey;
    ...
    .then(function (session) {
      ...
      //remove subscriptions not allowed
      if (session) return _this.reconfigureSessionSubscriptions(session);
    })
    .then(function (session) {

      if (session) callback(null, session);
    })
    .catch(callback);
};

SecurityService.prototype.reconfigureSessionSubscriptions = function (session) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.happn.services.subscription.allListeners(session.id)

      .then(function (references) {

        async.eachSeries(references, function (reference, referenceCallback) {

          _this.checkpoint._authorizeUser(session, reference.path, reference.action, function (e, authorized) {

            if (e) return referenceCallback(e);

            if (authorized) return referenceCallback();

            _this.happn.services.subscription.removeListener(reference.id, session.id, reference.action, reference.path, referenceCallback);
          });
        }, function (e) {
...
SecurityService.prototype.reconfigureSessions = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.sessionService.each(function (sessionData, sessionCallback) {
      ...
      try {
        ...
        _this.reconfigureSessionSecurity(sessionData, sessionCallback);
...
SecurityService.prototype.__initializeSecurityDirectyChanged = function (config, callback) {

  var _this = this;

  _this.securityDirectyChangedCheck = setInterval(function () {

    _this.dataService.get('/_SYSTEM/_SECURITY/DIRECTORY_CHANGED', function (e, changeKey) {

      if (e) return _this.happn.services.error.handleFatal('failure updating cached security data', e);

      if (changeKey && changeKey.data.value != _this.securityDirectyChangedKey) {

        _this.securityDirectyChangedKey = changeKey.data.value;

        _this.dataChanged(false, function(directoryChangedKey){
          _this.emit('security-data-changed', directoryChangedKey);
        });
      }
    });
  }, config.securityDirectoryChangedCheckInterval);

  callback();
};

SecurityService.prototype.setSecurityDirectyChangedKey = function () {

  var _this = this;

  _this.securityDirectyChangedKey = shortid.generate();

  return new Promise(function (resolve, reject) {

    _this.dataService.upsert('/_SYSTEM/_SECURITY/DIRECTORY_CHANGED', _this.securityDirectyChangedKey, function (e) {
...
SecurityService.prototype.dataChanged = function (updateDB, callback) {

  var _this = this;

  _this.users.clearCaches()
    .then(function () {
      return _this.checkpoint.clearCaches();
    })
    .then(function () {
      return _this.reconfigureSessions();
    })
    .then(function () {
      if (updateDB) return _this.setSecurityDirectyChangedKey();
      else return new Promise(function(resolve){
        resolve(_this.securityDirectyChangedKey);
      })
    })
    .then(function (directoryChangedKey) {
      _this.emit('security-data-updated', directoryChangedKey);
      if (callback) callback(directoryChangedKey);
    })
    .catch(function (e) {
      //no callback in here because the process dies
...
SecurityService.prototype.generateSession = function (user, sessionId, credentials, callback) {
  ...
  session.permissionSetKey = this.generatePermissionSetKey(session.user);
...
SecurityService.prototype.__loginFailed = function (username, specificMessage, e, callback, overrideLockout) {
  ...
  if (_this.config.accountLockout && _this.config.accountLockout.enabled && !overrideLockout) {
    ...
    return _this.__locks.get(username, function (e, currentLock) {
      ...
      callback(_this.happn.services.error.AccessDeniedError(message));
    });
  }

  return callback(_this.happn.services.error.AccessDeniedError(message));
};
      ...
      if (credentials.token) {
        ...
        try {
          ...
          return _this.authorize(previousSession, null, 'login', function (e, authorized, reason) {
            ...
            if (e) return _this.__loginFailed(previousSession.username, 'authorization failure', e, callback, true);
            ...
            _this.users.getUser(previousSession.username, function (e, user) {
              ...
              if (user == null) return _this.__loginFailed(previousSession.username, 'token user does not exist', null, callback, true);
        ...
        _this.users.getPasswordHash(credentials.username, function (e, hash) {
          ...
          _this.matchPassword(credentials.password, hash, function (e, match) {
            ...
            if (!match) return _this.__loginFailed(credentials.username, 'Invalid credentials', null, callback);
