...
function revokeSession(session, reason, callback) {
  ...
  if (!decoded.policy[0].ttl || decoded.policy[0].ttl === Number.POSITIVE_INFINITY) noTTL = true;
  else ttl += decoded.policy[0].ttl;

  if (!decoded.policy[1].ttl || decoded.policy[1].ttl === Number.POSITIVE_INFINITY) noTTL = true;
    ...
    try {
      ...
      config.profiles.push({
        ...
        policy: {
          ...
          inactivity_threshold: Number.POSITIVE_INFINITY
        }
      });
      ...
      config.profiles.forEach(profile => {
        if (profile.policy.ttl && profile.policy.ttl !== Number.POSITIVE_INFINITY)
          profile.policy.ttl = this.happn.services.utils.toMilliseconds(profile.policy.ttl);
        if (profile.policy.inactivity_threshold && profile.policy.ttl !== Number.POSITIVE_INFINITY)
