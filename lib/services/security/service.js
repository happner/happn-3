...
function processAuthorize(message, callback) {
  ...
  if (message.request.options && message.request.options.onBehalfOf)
...
function authorizeOnBehalfOf(session, path, action, onBehalfOf, callback) {
  ...
  var completeCall = (err, authorized, reason, onBehalfOfSession) => {
  ...
  this.authorize(session, path, action, (e, authorized) => {
    if (e) return completeCall(e, false, 'failed to check if session attempting to act on behalf of is authorized');
    if (!authorized) return completeCall(null, false, 'session attempting to act on behalf of is not authorized');
...
function __getOnBehalfOfSession(delegatedBy, onBehalfOf, callback){
  this.users.getUser(onBehalfOf, (e, onBehalfOfUser) => {
    if (e) return callback(e);
    if (!onBehalfOfUser) return callback(null, null);
    return this.generateSession(onBehalfOfUser, null, {info:{delegatedBy}}, callback);
  });
}
