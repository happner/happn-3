...
function revokeSession(session, reason, callback) {
  ...
  this.__cache_revoked_sessions.set(session.id, {
    ...
    if (!e) this.dataChanged('session-revoked', session, `session with id ${session.id} revoked`);
    callback(e);
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    this.sessionService.each((sessionData, sessionCallback) => {
      ...
      if (whatHappnd == 'session-revoked') {
        ...
        //fetch the revoked session to update the cache, if the session was revoked from another process
        return this.__cache_revoked_sessions.get(sessionData.id, sessionCallback);
      }
      ...
      if (whatHappnd == 'session-restored') {
        ...
        return this.__cache_revoked_sessions.remove(sessionData.id, { noPersist:true }, sessionCallback);
      }
    ...
    try {
      ...
      return this.authorize(previousSession, null, 'login', (e, authorized, reason) => {
        ...
        this.users.getUser(previousSession.username, (e, user) => {
          ...
          return this.__loginOK(credentials, user, sessionId, callback, previousSession.id);
        });
      });
