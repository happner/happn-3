...
SecurityService.prototype.processAuthorize = Promise.promisify(
  processAuthorize
);
SecurityService.prototype.processAuthorizeUnsecure = Promise.promisify(
  processAuthorizeUnsecure
);
...
SecurityService.prototype.processUnsecureLogin = Promise.promisify(
  processUnsecureLogin
);
...
function initialize(config, callback) {
  try {
    ...
    //1 minute
      ...
      config.defaultNonceTTL = this.happn.services.utils.toMilliseconds(
        config.defaultNonceTTL
      );
    ...
    if (config.accountLockout.enabled == null)
      config.accountLockout.enabled = true;
    ...
    if (config.accountLockout.enabled) {
      ...
      if (!config.accountLockout.retryInterval)
        config.accountLockout.retryInterval = 60 * 1000 * 10; //10 minutes
    }

    this.config = config;

    if (!this.config.secure)
      this.processAuthorize = this.processAuthorizeUnsecure;
...
function getAuditData(message) {
  ...
  var path = request.path ? request.path : request.action;
  ...
  if (
    ...
    !this.happn.services.utils.wildcardMatchMultiple(
      this.config.audit.paths,
      path
    )
  )
  ...
  if (message.request.action === 'get') {
    ...
    if (Array.isArray(message.response))
      data.response = message.response.length;
...
function processLogin(message) {
  ...
  return new Promise((resolve, reject) => {
    this.login(credentials, sessionId, message.request, (e, session) => {
      ...
      var attachedSession = this.happn.services.session.attachSession(
        sessionId,
        session
      );
      ...
      if (!attachedSession)
        return reject(
          new Error(
            'session with id ' + sessionId + ' dropped while logging in'
          )
        );
...
function processAuthorize(message, callback) {
  return this.authorize(
    ...
    (e, authorized, reason) => {
      ...
      if (!authorized)
        return callback(
          this.happn.services.error.AccessDeniedError('unauthorized', reason)
        );
  ...
  return new Promise((resolve, reject) => {
    this.dataService.get(
      ...
      (e, response) => {
        ...
        if (!response) {
          if (!config.keyPair)
            config.keyPair = this.cryptoService.serializeKeyPair(
              this.cryptoService.createKeyPair()
            );
          ...
          return this.dataService.upsert(
            ...
            (e, result) => {
              ...
              this._keyPair = this.cryptoService.deserializeKeyPair(
                result.data.value
              );
          ...
          try {
            ...
            var transformedKeyPair = this.cryptoService.serializeKeyPair(
...
function __initializeReplication() {
  ...
  this.happn.services.replicator.on(
...
function activateSessionManagement(logSessionActivity, callback) {
  ...
  if (!this.config.secure)
    return callback(
      new Error('session management must run off a secure instance')
    );
...
function __loadRevokedSessions(callback) {
  ...
  this.__cache_revoked_sessions = this.cacheService.new(
    'cache_revoked_sessions',
    config
  );
...
function __loadSessionActivity(callback) {
  ...
  this.__cache_session_activity = this.cacheService.new(
    'cache_session_activity',
    config
  );
  ...
  this.__cache_revoked_sessions.get(sessionId, function(e, item) {
    ...
    if (item)
      return callback(
        null,
        false,
        'session with id ' + sessionId + ' has been revoked'
      );
...
function revokeSession(session, reason, callback) {
  ...
  if (session == null || session == undefined)
    return callback(new Error('session not defined'));
  ...
  try {
    ...
    if (!decoded.policy[0].ttl || decoded.policy[0].ttl == Infinity)
      noTTL = true;
    else ttl += decoded.policy[0].ttl;

    if (!decoded.policy[1].ttl || decoded.policy[1].ttl == Infinity)
      noTTL = true;
    else ttl += decoded.policy[1].ttl;

    if (noTTL) {
      this.log.warn(
        'revoking a session without a ttl means it stays in the revocation list forever'
      );
      ttl = 0;
    }
...
function sessionFromRequest(req, options) {
  if (req.happn_session != null) return req.happn_session; //pulled this somewhere else up the call stack
...
function __listCache(cacheName, filter, callback) {
  ...
  if (!this[cacheName])
    return callback('cache with name ' + cacheName + ' does not exist');
  ...
  this[cacheName].all((e, allItems) => {
    ...
    try {
      if (filter)
        allItems = this.happn.services.cache.filterCacheItems(filter, allItems);
          ...
          try {
            ...
            if (whatHappnd == 'link-group') {
              if (
                ...
                sessionData.permissionSetKey = this.generatePermissionSetKey(
                  sessionData.user
                );
                effectedSessions.push(getEffectedSession(sessionData));
              }
            ...
            if (whatHappnd == 'delete-user') {
              var userName = changedData.obj._meta.path.replace(
                '/_SYSTEM/_SECURITY/_USER/',
                ''
              );
...
function emitChanges(whatHappnd, changedData, effectedSessions) {
  return new Promise((resolve, reject) => {
    try {
      ...
      if (effectedSessions)
        effectedSessionsSerialized = JSON.stringify(effectedSessions);
...
function dataChanged(whatHappnd, changedData, additionalInfo, callback) {
  ...
  this.users
    ...
    .then(() => {
      return this.resetSessionPermissions(
        whatHappnd,
        changedData,
        additionalInfo
      );
    ...
    .then(() => {
      return this.__replicateDataChanged(
        whatHappnd,
        changedData,
        additionalInfo
      );
    ...
    .catch(e => {
      this.happn.services.error.handleFatal(
        'failure updating cached security data',
        e
      );
    });
}
...
function generateSession(user, sessionId, credentials, callback) {
  //console.log('SS__________________________', sec.replace(/-/g, ''));
  ...
  session.permissionSetKey = this.generatePermissionSetKey(
    session.user,
    session
  );
    ...
    try {
      ...
      config.profiles.forEach(profile => {
        if (profile.policy.ttl && profile.policy.ttl != Infinity)
          profile.policy.ttl = this.happn.services.utils.toMilliseconds(
            profile.policy.ttl
          );
        if (
          profile.policy.inactivity_threshold &&
          profile.policy.ttl != Infinity
        )
...
function __checkLockedOut(username) {
  if (
    ...
    !this.config.accountLockout.enabled
  )
    return false;
  ...
  return (
    existingLock != null &&
    existingLock.attempts >= this.config.accountLockout.attempts
  );
}

function __loginFailed(
  username,
  specificMessage,
  e,
  callback,
...
function login(credentials, sessionId, request, callback) {
  ...
  if (
    !(
      (credentials.username && (credentials.password || credentials.digest)) ||
      credentials.token
    )
  )
    return callback(
      this.happn.services.error.InvalidCredentialsError('Invalid credentials')
    );
  ...
  if (this.__checkLockedOut(credentials.username))
    return callback(
      this.happn.services.error.AccessDeniedError('Account locked out')
    );
  ...
  return this.users.getUser(credentials.username, (e, user) => {
    ...
    return this.users.getPasswordHash(credentials.username, (e, hash) => {
      if (e) {
        if (
          e.toString() ==
          'Error: ' + credentials.username + ' does not exist in the system'
        )
          return this.__loginFailed(
            credentials.username,
            'Invalid credentials',
...
function createAuthenticationNonce(request, callback) {
  if (!request.publicKey)
    return callback(new Error('no public key with request'));
...
function verifyAuthenticationDigest(request, callback) {
  ...
  this.cacheService.get(
    ...
    (e, nonce) => {
      if (e) return callback(e);
      if (!nonce)
        return callback(new Error('nonce expired or public key invalid'));

      try {
        var verified = this.cryptoService.verify(
          nonce,
          request.digest,
          request.publicKey
        );
  ...
  var completeCall = (err, authorized, reason) => {
    ...
    if (this.config.logSessionActivity)
      this.__logSessionActivity(
        ...
        e => {
          if (e)
            this.log.warn('unable to log session activity: ' + e.toString());
        }
      );
  };
  ...
  this.__checkRevocations(session.id, (e, authorized, reason) => {
    ...
    this.checkpoint._authorizeSession(
      ...
      (e, authorized, reason, passthrough) => {
        ...
        if (passthrough || session.bypassAuthUser)
          return completeCall(null, true);
...
function validateName(name, validationType) {
  if (!name)
    throw new this.errorService.ValidationError('names cannot be empty');
...
function serialize(objectType, obj, options) {
  ...
  if (options && options.clone === false) {
    ...
    returnObj = this.utilsService.clone(obj.data);
  }
