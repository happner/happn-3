...
function initialize(config, callback) {
  ...
  if (config.lockTokenToLoginType == null) config.lockTokenToLoginType = false;
...
function revokeToken(token, reason, callback) {
  ...
  } else if (this.config.lockTokenToLoginType && decoded.loginType != null) {
    // we are checking if the token contains a loginType - to have backward compatibility
    // with old machine to machine tokens
    ttl = decoded.policy[decoded.loginType].ttl;
  } else {
...
function restoreToken(token, callback) {
  this.__cache_revoked_tokens.remove(token, e => {
    if (!e) this.dataChanged('token-restored', { token }, `token restored: ${token}`);
    callback(e);
  });
}
    ...
    if (whatHappnd === 'token-revoked') {
      ...
      if (!changedData.replicated) {
        ...
        this.sessionService.disconnectSessions(
          ...
          e => {
            if (e) return reject(e);
            resolve(effectedSessions); //means this is the originating process
          }
        );
        return;
      }
...
function generateToken(session, type) {
  ...
  if (type == null) decoupledSession.type = 0; //stateless type if not specified
...
function generateSession(user, sessionId, credentials, tokenLogin) {
...
function __loginOK(credentials, user, sessionId, callback, tokenLogin) {
...
function __loginFailed(username, specificMessage, e, callback, overrideLockout) {
  ...
  if (this.config.accountLockout && this.config.accountLockout.enabled && !overrideLockout) {
    ...
    return callback(this.happn.services.error.AccessDeniedError(message));
  }
...
function login(credentials, sessionId, request, callback) {
  ...
  if (credentials.loginType == null) credentials.loginType = 1;
  ...
  if (credentials.token) {
    try {
      return this.__checkRevocations(credentials.token, (e, authorized, reason) => {
        ...
        if (previousSession.loginType != null && this.config.lockTokenToLoginType) {
          if (previousSession.loginType !== credentials.loginType)
            return callback(
              this.happn.services.error.AccessDeniedError(
                `token was created using the login type ${previousSession.loginType},
