...
SecurityService.prototype.getAuditData = function(message){
  ...
  if (this.config.audit.paths)
    if (!this.happn.services.utils.wildcardMatchMultiple(this.config.audit.paths, message.request.path?message.request.path:message.request.action)) return null;
  ...
  data.session = {type:message.session.type, username:message.session.user.username, protocol:message.session.protocol};
...
SecurityService.prototype.doAudit = Promise.promisify(function(message, callback){
  ...
  if (auditData != null){
    ...
    _this.dataService.upsert(auditPath, auditData, {set_type:'sibling'}, function(e){
...
SecurityService.prototype.doUnsecureLogin = Promise.promisify(function(message, callback){
  ...
  message.response = {data:this.happn.services.session.attachSession(message.session.id, session)};
...
SecurityService.prototype.processNonceRequest = function(message, callback){
  ...
  return this.createAuthenticationNonce(message.request.data, function(e, nonce){
    ...
    message.response = {
      nonce:nonce,
      data:{nonce:nonce}//happn-2 backward compatability
    };
...
SecurityService.prototype.revokeSession = function(session, reason, callback){
  ...
  try{
    ...
    this.__cache_revoked_sessions.set(session.id, {reason:reason, id:session.id}, {ttl:ttl}, function(e){
      callback(e);
    });
...
SecurityService.prototype.__loginFailed = function(username, specificMessage, e, callback){
  ...
  if (e) message = message + ': ' + e.toString();
    ...
    if ((credentials.username && (credentials.password || credentials.digest)) || credentials.token) {
      ...
      if (credentials.token){
        ...
        try{
          ...
          //if (previousSession.username != credentials.username) throw new Error('bad token');

          if (previousSession.disallowLogins)
            throw new Error('logins with this token are disallowed by policy');

          if (previousSession.lockToOrigin && previousSession.origin != _this.happn.services.system.name)
            throw new Error('this token is locked to a different origin by policy');

          return _this.authorize(previousSession, null, 'login', function(e){

            if (e) return _this.__loginFailed(previousSession.username, 'invalid token', null, callback);

            _this.users.getUser(previousSession.username, function (e, user) {

              if (user == null) return _this.__loginFailed(previousSession.username, 'token user does not exist', null, callback);
          ...
          return _this.__loginFailed(credentials.username, 'invalid credentials', e, callback);
        }
