...
function __loginOK(credentials, user, sessionId, callback) {
  ...
  this.__locks.removeSync(user.username);//remove previous locks
...
function __checkLockedOut(username) {
  ...
  if (!this.config.accountLockout || !this.config.accountLockout.enabled) return callback(null, false);
