...
function __ensureAdminUser (config, callback) {
      ...
      _this.sessionService.each(function (sessionData, sessionCallback) {
        ...
        try {
          ...
          if (whatHappnd == 'permission-removed') {
            ...
            if (sessionData.user.groups[whatHappnd.groupName] != null) {
              //all we need to do, permissionSetKey remains the same - all caches are cleared, but effected sessions are different
...
function dataChanged (whatHappnd, changedData, additionalInfo, callback) {
  ...
  if (typeof additionalInfo == 'function') callback = additionalInfo;
