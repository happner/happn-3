...
module.exports = class PermissionsTree {
  ...
  static create(permissionsObj) {
    console.log("CREATING FROM ", permissionsObj)
  ...
  buildPermissionList(pathArray, action, tree) {
    ...
    if (pathArray.length === 0) {
      const leaf = this.checkForLeaf(tree, action);
    ...
    if (pathArray[0] === '**') {
      if (tree['*']) {
        const leaf = this.checkForLeaf(tree['*'], action);
        if (leaf) return { allowed: [leaf], prohibited: this.buildPermission(tree, action, true) };
        if (this.checkProhibited(tree['*'], action)) return { allowed: [], prohibited: [] };
      }

      return {
        allowed: this.buildPermission(tree, action, false),
        prohibited: this.buildPermission(tree, action, true)
      };
    }
  ...
  buildPermission(tree, action, buildProhibited) {
    ...
    for (const branch of treeKeys) {
      ...
      if (!buildProhibited && '*' in tree && this.checkProhibited(tree['*'], action)) continue;
      permissions.push(...this.buildPermission(tree[branch], action, buildProhibited));
    }

    if (tree.$leaf) {
      if (buildProhibited && this.checkProhibited(tree, action)) permissions.push(tree.$leaf);
      if (!buildProhibited && !this.checkProhibited(tree, action)) permissions.push(tree.$leaf);
    }
