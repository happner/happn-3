...
module.exports = class PermissionsTree {
  ...
  mergeBranch(permissions) {
    return (tree, path) =>
      _.setWith(tree, path.split('/').slice(path.startsWith('/') ? 1 : 0), {
        $leaf: path,
        ...permissions[path]
      }, Object);
  }
  ...
  wildcardPathSearch(permissionPath, action, tree, depth) {
    ...
    if (pathArray.indexOf('**') < pathArray.length - 1) {
  ...
  checkForLeaf(tree, action) {
    if (
      ...
      tree.actions && tree.actions.includes(action)
    ) {
      return tree.$leaf;
    }
    return null;
    ...
    if (pathArray[0] == '*') {
      ...
      if (pathArray.length == 1) {
        Object.keys(tree).forEach(branch => {
          ...
          if (leaf) permissionPaths = [...permissionsPaths, leaf];
        });
