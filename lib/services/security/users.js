...
SecurityUsers.prototype.__stripGroups = __stripGroups;
SecurityUsers.prototype.linkGroupsToUser = linkGroupsToUser;
...
function __stripGroups(user) {
  var clonedUser = this.utilsService.clone(user);
  delete clonedUser.groups;
  return clonedUser;
}
...
function getUserNoGroups(userName, callback) {
  ...
  if (this.__cache_users.has(preparedUserName))
    return callback(null, this.__stripGroups(this.__cache_users.getSync(preparedUserName)));
  ...
  this.dataService.get(userPath, (e, found) => {
    ...
    this.__cache_users.setSync(preparedUserName, returnUser, { clone: false });
    this.__cache_passwords.setSync(preparedUserName, password, { clone: false });
    callback(null, returnUser);
  });
}

function linkGroupsToUser(user, callback) {
  if (user.groups) return callback(null, user);
  user.groups = {};
  ...
  this.dataService.get(
    ...
    (e, userGroups) => {
      ...
      this.__cache_users.setSync(user.username, user, { clone: false });
      callback(null, user);
    }
  );
}
...
function getUser(userName, options, callback) {
  ...
  this.getUserNoGroups(userName, (e, user) => {
    ...
    return this.linkGroupsToUser(user, callback);
  });
}
