...
SecurityUsers.prototype.__getUserNames = __getUserNames;
...
function __getUserNamesFromGroupLinks(userGroupLinks){
  return userGroupLinks.map(function(link){
    return link._meta.path.split('/_SYSTEM/_SECURITY/_USER/')[0].split('/_USER_GROUP/')[0];
  });
};

function __getUserNamesFromGroupName(groupName){

  var _this = this;

  return new Promise(function(resolve, reject){
    _this.dataService.get('/_SYSTEM/_SECURITY/_USER/*/_USER_GROUP/' + groupName, {path_only:true})
      .then(function(e, userGroupLinks){
        if (e) return reject(e);
        if (userGroupLinks.paths.length == 0) return resolve([]);
        resolve(_this.__getUserNamesFromGroupLinks(userGroupLinks.paths));
      })
  });
}
...
function listUsersByGroup(groupName, options, callback) {
  ...
  if (typeof groupName != "string") return callback('validation error: groupName must be specified');
  ...
  _this.__getUserNamesFromGroupName(groupName)
    .then(function(usernames){

      userSearchCriteria.$and.push({username{$in:usernames}});
      _this.dataService.get(searchPath, userSearchCriteria, function (e, results) {
