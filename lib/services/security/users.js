...
function listUsers(userName, options, callback) {
  ...
  if (userName === '*' && noCriteria && this.__cache_list_all_users != null)
    return callback(null, this.utilsService.clone(this.__cache_list_all_users));
  ...
  this.dataService.get(searchPath, { criteria: options.criteria }, (e, results) => {
    ...
    if (userName === '*' && noCriteria)
      this.__cache_list_all_users = this.utilsService.clone(returnUsers); //update our cache
