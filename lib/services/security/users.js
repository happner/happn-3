...
var ensureAsync = require('dezalgo');
...
function initialize(config, securityService, callback) {
  ...
  try {
    ...
    this.__cache_users = this.cacheService.new('cache_security_users', {type: 'LRU', cache: config.__cache_users});
    this.__cache_passwords = this.cacheService.new('cache_security_passwords', {
      type: 'LRU',
      cache: config.__cache_users
    });

    this.__cache_list_all_users = null;
  ...
  if (validationType == 'user') {
    ...
    if (!obj.password && !obj.publicKey) {
      ...
      return this.dataService.get('/_SYSTEM/_SECURITY/_USER/' + obj.username, {}, (e, result) => {
        ...
        this.securityService.checkOverwrite(validationType, obj, '/_SYSTEM/_SECURITY/_USER/' + obj.username, obj.username, options, callback);
      });
    }
...
function __prepareUser(user, options, callback) {
  ...
  return this.cryptoService.generateHash(user.password, (e, hash) => {
    if (e) return callback(e);
...
function __upsertUser(user, options, callback) {
  ...
  this.__prepareUser(user, options, (e, prepared) => {
...
function deleteUser(user, options, callback) {
  ...
  this.dataService.remove(userTree, {}, (e, result1) => {
    ...
    this.dataService.remove(userPath, {}, (e, result2) => {
      ...
      this.__cache_users.remove(user.username)
        .then(() => {
...
function listUsers(userName, options, callback) {
  ...
  this.dataService.get(searchPath, {criteria: options.criteria}, (e, results) => {

    if (e) return callback(e);

    var returnUsers = this.securityService.serializeAll('user', results, options);

    if (userName == "*" && noCriteria) this.__cache_list_all_users = this.utilsService.clone(returnUsers);//update our cache
...
function listUserNamesByGroup(groupName){
  ...
  return new Promise((resolve, reject) => {
    ...
    this.dataService.get('/_SYSTEM/_SECURITY/_USER/*/_USER_GROUP/' + groupName, {path_only:false}) // TODO return to true once #180 is resolved
      .then(userGroupLinks => {
...
function listUsersByGroup(groupName, options, callback) {
  ...
  this.listUserNamesByGroup(groupName)
    .then(usernames => {
      ...
      this.dataService.get('/_SYSTEM/_SECURITY/_USER/*', {criteria: userSearchCriteria}, (e, results) => {
        if (e) return callback(e);
