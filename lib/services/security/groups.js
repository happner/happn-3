...
SecurityGroups.prototype.upsertLookupPermission = upsertLookupPermission;
SecurityGroups.prototype.removeLookupPermission = removeLookupPermission;
...
function upsertLookupPermission(groupName, permission) {
  let permissions = this.dataService.upsert(
    ['/_SYSTEM/_SECURITY/_PERMISSIONS/_LOOKUP', groupName, permission.table].join('/'),
    permission
  ); // What about multiple permissions on the same group/table combo? To be dealt with...
}
function removeLookupPermission(groupName, permission) {}
