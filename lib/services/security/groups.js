...
function clearCaches() {
  ...
  this.__cache_permissions.clear();
}
...
function listGroups(groupName, options, callback) {
  ...
  try {
    ...
    this.dataService.get(searchPath, {
      ...
      if (!options.skipPermissions) _this.__attachPermissionsAll(groups)
    ...
    _this.listPermissions(group.name)
      .then(function (permissions) {
        ...
        permissions.forEach(function (item) {
          ...
          var permission = item.data;

          if (permission.authorized) {
            var permissionPath = item._meta.path.replace('/_SYSTEM/_SECURITY/_PERMISSIONS/' + group.name + '/' + permission.action + '/', '');
            if (!group.permissions[permissionPath]) group.permissions[permissionPath] = {actions: []};
            group.permissions[permissionPath].actions.push(permission.action);
          }
        });
...
function listPermissions(groupName) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (_this.__cache_permissions.has(groupName)) return _this.__cache_permissions.get(groupName);
    ...
    _this.dataService.get(searchPath, {
      ...
      _this.__cache_permissions.set(groupName, results);
  ...
  return new Promise(function (resolve, reject) {
    ...
    return _this.__removePermission(groupName, path, action)
      .then(function (result) {
        ...
        if (result.removed) {
          _this.securityService.dataChanged('permission-removed', {
            groupName: groupName,
            path: path,
            action: action
          }, null, function () {
...
function __removePermission(groupName, path, action) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.dataService.remove(['/_SYSTEM/_SECURITY/_PERMISSIONS', groupName, action, path].join('/'), function (e, result) {
...
function upsertPermission(groupName, path, action, authorized) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    return _this.__upsertPermission(groupName, path, action)
      .then(function (result) {
...
function validateGroupPermissions(permissions) {
  ...
  Object.keys(permissions).forEach(function (permissionPath) {
    ...
    if (!permission.actions) return errors.push('missing actions: ' + permissionPath);

    permission.actions.forEach(function (action) {
      if (['set', 'get', 'remove', 'on', '*'].indexOf(action) == -1) return errors.push('unknown action: ' + action + ' for path: ' + permissionPath);
    });
  });
...
function __upsertPermission(groupName, path, action, authorized) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (!groupName) return reject(new Error('please supply a groupName'));
    if (!path) return reject(new Error('please supply a path'));
    if (!action) action = '*';
    if (authorized == null) authorized = true;

    authorized = !!authorized;//must always be stored true or false

    _this.dataService.upsert(['/_SYSTEM/_SECURITY/_PERMISSIONS', groupName, action, path].join('/'), {
      action: action,
      authorized: authorized
    }, function (e, result) {
      if (e) return reject(e);
