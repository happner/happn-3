module.exports = {
  ...
  wildcardMatch: function(pattern, matchTo) {
    ...
    const preparedPattern = this.prepareWildPath(pattern);
    //one is * or ** or *** etc
    if (preparedPattern === "*" || preparedPattern === matchTo) return true;
    //starts with reject
    const initialSegment = preparedPattern.split("*").pop();
