...
module.exports = {
    ...
    wildcardMatch: function (pattern, matchTo) {
      ...
      pattern = this.prepareWildPath(pattern);//replace **,***,**** with *

      //one is * or ** or *** etc
      if (pattern == '*') return true;//one is anything

      if (pattern == matchTo) return true;//equal to each other

      return this.makeRe(pattern).test(matchTo);
    },
