module.exports = {
  ...
  checkPath: function (path, action) {
    ...
    if (action == 'set' && path.indexOf('*') > -1)
      throw 'Bad path, if the action is \'set\' the path cannot contain the * wildcard character';

    if (path.match(/^[a-zA-Z0-9/(/)/+/=/&/:%@.//_*/-\s]+$/) == null)
      throw 'Bad path, can only contain characters a-z A-Z 0-9 / & + = : @ % * ( ), ie: factory1@I&J(western-cape)/plant1:conveyer_2/stats=true/capacity=10%/*';
  },
