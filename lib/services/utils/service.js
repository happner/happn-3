...
function ensureRegexCache(cacheKey, cacheSize) {
...
function wildcardMatch(pattern, matchTo, cacheKey, cacheSize) {
  ...
  const cache = this.ensureRegexCache(cacheKey, cacheSize);
  if (!cache.has(preparedPattern)) cache.set(preparedPattern, happnUtils.makeRe(preparedPattern));
  const regex = cache.get(preparedPattern);
  regex.lastIndex = 0;
