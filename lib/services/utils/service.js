...
UtilsService.prototype.wildcardMatch = happnUtils.wildcardMatch.bind(
  happnUtils
);
        ...
        if (this.wildcardMatch(sortedKeys[wcPathIndex], wcPathCompare)) {
          ...
          if (wildcardDict[sortedKeys[wcPathIndex]].actions.indexOf('*') == -1)
            for (var actionIndex in wildcardDict[wcPathCompare].actions) {
              ...
              if (
                wildcardDict[sortedKeys[wcPathIndex]].actions.indexOf(action) ==
                -1
              ) {
...
UtilsService.prototype.stringContainsAny = function (
  str,
  segments,
  caseInSensitive
) {
...
UtilsService.prototype.getFirstMatchingProperty = function (properties, obj) {
  ...
  for (var propertyName in obj)
    if (checkProperties.indexOf(propertyName.toLowerCase()) > -1)
