...
ErrorService.prototype.handleSystem = function (e, area, severity, callback) {
  ...
  if (systemError.stack)//TODO: regarding below: log.write(': ' - fix happn-logger - then fix this back to not prepend :
    return this.happn.services.log.write(': ' + systemError.message, 'error', 'system error: ' + area + ', stack: ' + systemError.stack, null, null, function () {
      if (typeof callback == 'function') {
        return callback(systemError);
      }
    });
  this.happn.services.log.write(': ' + systemError.message, 'error', 'system error: ' + area, null, null, function () {
    if (typeof callback == 'function') {
      return callback(systemError);
    }
  });
