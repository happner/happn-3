(function() { // begin enclosed
  ...
  HappnClient.prototype.handle_data = function(path, message) {
    ...
    if (this.state.events[path].length == 1)
      return this.delegate_handover(message, this.state.events[path][0]);
  ...
  HappnClient.prototype._offListener = function(handle, callback) {
    ...
    Object.keys(_this.state.events).every(function(channel) {
      ...
      return listeners.every(function(listener) {
        ...
        if (_this.state.refCount[listener.eventKey] == 0 || listener.initialEmit || listener.initialCallback) _this._remoteOff(channel, _this.__getListenerRef(listener), function(e){
          if (e) return callback(e);
          delete _this.state.refCount[listener.eventKey];
          _this.__clearListenerRef(listener);
          if (listeners.length == 0) delete _this.state.events[channel];
          callback();
        });
        else callback();
