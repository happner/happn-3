(function () { // begin enclosed
  ...
  HappnClient.prototype.__prepareOptions = function(options){
    ...
    //if (!options.config.connectTimeout) options.config.connectTimeout = 300000; //5 minutes
  ...
  HappnClient.prototype.__getConnection = function(errorHandler, callback){
    ...
    try{
      ...
      if (browser) socket = Primus.connect(_this.options.config.url, _this.options.config.pubsub.options);
        ...
        socket = new Socket(_this.options.config.url, _this.options.config.pubsub.options);
      }

      socket.on('open', function () {

        if (_this.connectTimeout) clearTimeout(_this.connectTimeout);
      ...
      if (_this.options.config.connectTimeout > 0){
        ...
        _this.connectTimeout = setTimeout(function(){
          ...
          try{
            ...
            _this.log.error('failed ending timed out connection socket', e);
          }
        ...
        }, _this.options.config.connectTimeout);
      }
    }catch(e){
      callback(e);
    }
