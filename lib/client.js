(function() { // begin enclosed
  ...
  HappnClient.__isRanged = function(connection) {
  ...
  HappnClient.__getRange = function(connection) {
  ...
  HappnClient.__preparePoolOptions = function(connection, options) {

    if (options == null) options = {};

    if (Array.isArray(connection)) options.pool = connection;

    if (this.__isRanged(connection)) options.pool = this.__getRange(connection);
  ...
  HappnClient.create = Promisify(function(connection, options, callback) {
    ...
    try {
      clientOptions = HappnClient.__preparePoolOptions(connection, options);
    } catch (e) {
      return callback(e);
    }
  ...
  HappnClient.prototype.__getConnection = function(callback){
    ...
    try {
      ...
      if (this.options.pool) {
        ...
        if (!this.__tried) this.__initializeConnectionPool(true);
        else this.__poolAttempts++;

        if (this.options.poolReconnectAttempts > 0 && (this.__poolAttempts > this.options.poolReconnectAttempts)) {
          return callback(new Error('pool reconnection attempts exceeded'));
        }

        if (this.__possible.length == 0) { //what is possible is now zero, start all over again until we get a connection or we have reconnected successfully
          this.__initializeConnectionPool();
          delayConnection = true;
        }

        var possibleIndex;

        if (this.options.poolType == this.constants.CONNECTION_POOL_TYPE.RANDOM)
          possibleIndex = Math.floor(Math.random() * this.__possible.length);

        if (this.options.poolType == this.constants.CONNECTION_POOL_TYPE.ORDERED)
          possibleIndex = 0;

        this.__updateOptions(this.__possible[possibleIndex]);

        this.__tried.push(this.__possible[possibleIndex]);

        this.__possible.splice(possibleIndex, 1); //remove from the list of possibilities

        var _this = this;
        if (delayConnection) return setTimeout(function(){
          _this.__connectSocket(callback);
        }, this.options.poolReconnectDelay);
      }
  ...
  HappnClient.prototype.__initializeConnectionPool = function(resetAttempts) {

    this.__tried = [];

    this.__possible = this.options.pool.map(function(connection) {
      return connection;
    });

    if (resetAttempts) this.__poolAttempts = 0;
  };
  ...
  HappnClient.prototype.__connectSocket = function(callback) {
    ...
    socket.on('open', function waitForConnection() {
      ...
      if (_this.status === STATUS.CONNECTING) {
        ...
        if (_this.options.pool) _this.__initializeConnectionPool(true);
  ...
  HappnClient.prototype.__attachPublishedAck = function(options, message) {
    ...
    if (typeof options.onPublished != 'function') throw new Error('onPublished handler in options is missing');
  ...
  HappnClient.prototype.handle_ack = function(message) {
    ...
    if (this.state.ackHandlers[message.id]) {
      ...
      if (message.status == 'error') return this.state.ackHandlers[message.id].handle(new Error(message.error), message.result);
