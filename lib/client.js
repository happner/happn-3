(function () { // begin enclosed
  ...
  HappnClient.prototype.__prepareSecurityOptions = function(options){
    ...
    if (options.token) options.config.token = options.token;
  ...
  HappnClient.prototype.__prepareSocketOptions = function(options){
    ...
    if (options.config.pubsub) options.socket = options.config.pubsub.options;
    ...
    if (options.config.connectTimeout) options.socket.timeout = options.config.connectTimeout;//default is 15 seconds

    if (options.connectTimeout) options.socket.timeout = options.connectTimeout;

    if (!options.socket.timeout) options.socket.timeout = 15000;//default

    if (options.socket.reconnect.strategy) options.socket.strategy = options.socket.reconnect.strategy;

    if (!options.socket.strategy) options.socket.strategy = "disconnect,online";
  ...
  HappnClient.prototype.__prepareOptions = function(options){
    ...
    this.__prepareSecurityOptions(options);
    ...
    this.__prepareSocketOptions(options);
  ...
  HappnClient.prototype.__getConnection = function(errorHandler, callback){
    ...
    try{
      ...
      if (browser)
        socket = new Primus(_this.options.config.url, _this.options.socket);

      else {

        Primus = require('primus'),
          Socket = Primus.createSocket({
            transformer: _this.options.config.transformer,
            parser: _this.options.config.parser,
            manual: true
          });
        socket = new Socket(_this.options.config.url, _this.options.socket);
      }
  ...
  HappnClient.prototype.authenticate = Promisify(function (callback) {
    ...
    _this.__getConnection(_this.handle_error, function(e, pubsub){
      ...
      _this.pubsub.on('reconnected', _this.reconnect.bind(_this));
      _this.pubsub.on('end', _this.handle_end.bind(_this));
      _this.pubsub.on('close', _this.handle_end.bind(_this));
      _this.pubsub.on('reconnect timeout', _this.handle_reconnect_timeout.bind(_this));
      _this.pubsub.on('reconnect scheduled', _this.handle_reconnect_scheduled.bind(_this));
