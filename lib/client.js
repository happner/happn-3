...
(function() {
  ...
  HappnClient.prototype.__setCookieCheckTimer = function(session) {
    ...
    this.cookieInterval = setInterval(() => {
      let currentCookie = this.__getCookieInstance(session.cookieName || 'happn_token');
  ...
  HappnClient.prototype.__getCookieInstance = function(cname) {
    const name = `${cname}=`;
