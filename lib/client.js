...
(function() {
  ...
  HappnClient.prototype.__writeCookie = function(session, sessionDocument) {
    ...
    if (this.cookieEventHandler) {
      this.cookieEventHandler('cookie-write', cookie);
    }
  };

  HappnClient.prototype.__setCookieCheckTimer = function(session, sessionDocument) {
    if (typeof this.cookieEventHandler !== 'function') {
      return;
    }
    const cookieEventInterval =
      isNaN(this.cookieEventInterval) || parseInt(this.cookieEventInterval) < 500
        ? 1000 // if not a number or < 500 set to sane default
        : parseInt(this.cookieEventInterval);
    if (this.cookieInterval) {
      clearInterval(this.cookieInterval); // clear any previous intervals
    }

    this.cookieStatus = 'logged_in'; // this only happens on authentication
    this.initialCookie = this.__getCookieInstance(session.cookieName || 'happn_token');
    this.cookieEventHandler('cookie-created', this.initialCookie);

    this.cookieInterval = setInterval(() => {
      let currentCookie = this.__getCookieInstance(
        session.cookieName || 'happn_token',
        sessionDocument
      );
      if (currentCookie === '' && this.cookieStatus === 'logged_in') {
        this.cookieStatus = 'logged_out';
        this.cookieEventHandler('cookie-deleted', this.initialCookie);
      } else if (currentCookie.length > 0 && this.cookieStatus === 'logged_out') {
        this.cookieStatus = 'logged_in';
        this.cookieEventHandler('cookie-created', currentCookie);
      }
    }, cookieEventInterval);
  };

  HappnClient.prototype.__getCookieInstance = function(cname, sessionDocument) {
  ...
  HappnClient.prototype.__expireCookie = function(session, sessionDocument) {
    ...
    sessionDocument.cookie = cookie;
    if (this.cookieEventHandler) {
      this.cookieEventHandler('cookie-expired', cookie);
    }
  };
  ...
  HappnClient.prototype.__attachSession = function(result) {
    ...
    if (browser) {
      ...
      this.__setCookieCheckTimer(result);
    }
  };
  ...
  HappnClient.prototype.__reattachChangedListeners = function(permissions) {
    ...
    let errors = [];
    listenerPermPaths.forEach(path => {
      let listeners = this.state.events[path];
      if (listeners) {
        listeners.forEach(listener => {
          this.state.refCount[listener.eventKey] = 0;
          if (!this.state.events[path]) return; // We  have deleted this key.
          ...
          this._offPath(path, e => {
            if (e)
              errors.push(
                new Error('failed detaching listener to path, on re-establishment: ' + path, e)
              );
            ...
            this._remoteOn(path, parameters, (e, response) => {
              if (e) {
                if ([403, 401].indexOf(e.code) > -1) {
                  ...
                  delete this.state.events[path];
                  return;
                }
                errors.push(new Error('failed re-establishing listener to path: ' + path, e));
                return;
              }
        ...
        this._remoteOn(path, {}, (e, response) => {
          if (e) {
            if ([403, 401].indexOf(e.code) > -1) {
              //permissions may have changed regarding this path
              delete this.state.events[path];
              this.__clearSecurityDirectorySubscriptions(path);
            }
          } else {
            this._remoteOff(path, response.id, () => {});
          }
        });
      }
    });
  ...
  HappnClient.prototype.__updateSecurityDirectory = function(message) {
    ...
    if (message.data.whatHappnd === _this.CONSTANTS.SECURITY_DIRECTORY_EVENTS.PERMISSION_REMOVED) {
      if (['*', 'on'].indexOf(message.data.changedData.action) > -1) {
        ...
        return _this.__reattachChangedListeners(permissions);
      }
    }
    ...
    if (message.data.whatHappnd === _this.CONSTANTS.SECURITY_DIRECTORY_EVENTS.UPSERT_GROUP) {
      Object.keys(message.data.changedData.permissions).forEach(function(permissionPath) {
        ...
        if (
          ...
          _this.__clearSecurityDirectorySubscriptions(permissionPath);
      });
    }

    if (message.data.whatHappnd === _this.CONSTANTS.SECURITY_DIRECTORY_EVENTS.UNLINK_GROUP) {
      return _this.__reattachChangedListeners(message.data.changedData.permissions);
    }
  ...
  HappnClient.prototype.__cookieCleanup = function(sessionDocument) {
    ...
    if (typeof this.cookieEventHandler === 'function') {
      clearInterval(this.cookieInterval); // we are intentionally deleting the cookie
      this.cookieEventHandler('cookie-deleted', this.initialCookie);
    }
  };
