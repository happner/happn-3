(function() { // begin enclosed
  ...
  HappnClient.prototype.__initializeEvents = function() {
    ...
    _this.emit = function(eventName, eventData) {
      ...
      if (_this.eventHandlers[eventName]) {
        _this.eventHandlers[eventName].forEach(function(handler) {
          if (!handler) return;
  ...
  HappnClient.prototype.handle_data = function(path, message) {
    ...
    if (this.events[path].length == 1)
      return this.delegate_handover(message, this.events[path][0]);

    var intermediateMessage = JSON.stringify(message);
    var _this = this;
    _this.events[path].forEach(function(delegate) {
  ...
  HappnClient.prototype.clearTimeouts = function(){
    var _this = this;
    Object.keys(_this.__ackHandlers).forEach(function(handlerKey){
      clearTimeout(_this.__ackHandlers[handlerKey].timedout);
    });
    Object.keys(_this.requestEvents).forEach(function(handlerKey){
      clearTimeout(_this.requestEvents[handlerKey].timedout);
    });
  };
