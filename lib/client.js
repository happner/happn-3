(function () { // begin enclosed
  ...
  HappnClient.__isRanged = function(connection){
    ...
    if (connection && connection.config){
      ...
      if (connection.config.host && connection.config.host.range != null){

        if (!Array.isArray(connection.config.host.range) || connection.config.host.range.length != 2)
          throw new Error('invalid range option, range must be an array or length must be 2');

        hostRanged = true;
      }

      if (connection.config.port && connection.config.port.range != null){

        if (!Array.isArray(connection.config.port.range) || connection.config.port.range.length != 2)
          throw new Error('invalid range option, range must be an array or length must be 2');
  ...
  HappnClient.__getRange = function(connection){
    ...
    var serializedOptions = JSON.stringify(connection);

    if (!connection.config.port) connection.config.port = 55000;

    if (!connection.config.host) connection.config.host = '127.0.0.1';

    var appendConnectionOptions = function(host, port){

      var options = JSON.parse(serializedOptions);

      options.config.host = host;
      options.config.port = port;

      connectionRange.push(options);
    };

    if (connection.config.host.range != null){

      var fromOctet = connection.config.host.range[0].split('.');

      var toOctet = connection.config.host.range[1].split('.');

      var first = fromOctet[3];
      var last = toOctet[3];

      for (var i = first;i <= last; i++){
        if (i < 256) appendConnectionOptions(fromOctet.slice(0,3).join('.') + '.' + i, connection.config.port);
      }
    }

    if (connection.config.port.range != null){

      for (var i = connection.config.port.range[0];i <= connection.config.port.range[1];i++){
        appendConnectionOptions(connection.config.host, i);
      }
    }
  ...
  HappnClient.prototype.__prepareConnectionOptions = function (options) {
  ...
  HappnClient.prototype.__prepareOptions = function (options) {
    ...
    if (preparedOptions.pool) {
      ...
      preparedOptions.pool = preparedOptions.pool.map(function (connection) {
        ...
        return _this.__prepareConnectionOptions(connection.config ? connection.config : connection);
      });
    ...
    _this.__getConnection = function (callback) {
      ...
      try {
        ...
        if (_this.options.pool) {
          ...
          _this.options.url = _this.__possible[possibleIndex].url;
  ...
  HappnClient.prototype.disconnect = Promisify(function (options, callback) {
    ...
    if (_this.state == STATE.DISCONNECTED) {
      if (_this.socket) _this.socket.end(); // end() anyway to ensure reconnect loop is stopped
      return callback();
    }
    ...
    _this.disconnectTimeout = setTimeout(function () {
      ...
      if (_this.state !== STATE.DISCONNECTING && _this.socket != null) _this.socket.end();//try again, the disconnect call never came back
