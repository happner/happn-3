(function () { // begin enclosed
    ...
    _this.__getConnection = function (callback) {
      ...
      try {
        ...
        if (_this.options.pool) {
          ...
          if (_this.__poolAttempts > _this.options.poolReconnectAttempts) return callback(new Error('pool reconnection attempts exceeded'));
  ...
  HappnClient.prototype.disconnect = Promisify(function (options, callback) {
    ...
    if (_this.state == STATE.DISCONNECTED) {
      _this.socket.end(); // end() anyway to ensure reconnect loop is stopped
      return callback();
    }
