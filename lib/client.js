(function() { // begin enclosed
  ...
  HappnClient.prototype.__updateSecurityDirectorySubscriptions = function(message){
    var _this = this;
    Object.keys(this.state.events).forEach(function(eventListenerPath){
      if (message.data.changedData.path == eventListenerPath.substring(eventListenerPath.indexOf('@') + 1)){
  ...
  HappnClient.prototype.__updateSecurityDirectory = function(message){
    if (message.data.whatHappnd == 'permission-removed') {
      if (['*','on'].indexOf(message.data.action)) this.__updateSecurityDirectorySubscriptions(message);
    }
  };
