(function () { // begin enclosed
  ...
  HappnClient.prototype.__prepareSocketOptions = function (options) {
    ...
    // set ping and pong options (otherwise fall back to happn-primus defaults)
    if (typeof options.ping != 'undefined') {
      options.socket.ping = options.ping;
    }
    if (typeof options.pong != 'undefined') {
      options.socket.pong = options.pong;
    }
  ...
  HappnClient.prototype.__connectSocket = function (callback) {
    ...
    if (browser) socket = new Primus(_this.options.url, _this.options.socket);
      ...
      console.log('INIT OPTIONS', _this.options);
