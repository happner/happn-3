(function () {
  ...
  var __isRanged = function (connection) {
    ...
    if (checkConnection.host && checkConnection.host.range != null) {
      if (
        !Array.isArray(checkConnection.host.range) ||
        checkConnection.host.range.length != 2
      )
        throw new Error(
          'invalid range option, range must be an array or length must be 2'
        );
    ...
    if (checkConnection.port && checkConnection.port.range != null) {
      if (
        !Array.isArray(checkConnection.port.range) ||
        checkConnection.port.range.length != 2
      )
        throw new Error(
          'invalid range option, range must be an array or length must be 2'
        );
    ...
    if (hostRanged && portRanged)
      throw new Error(
        'invalid range option, range can only be by host or port, not both'
      );
  ...
  var __getRange = function (connection) {
    ...
    if (rangeConnection.host.range != null) {
      ...
      for (var i = first; i <= last; i++) {
        if (i < 256)
          appendConnectionOptions(
            fromOctet.slice(0, 3).join('.') + '.' + i,
            rangeConnection.port
          );
      }
    }
    ...
    if (rangeConnection.port.range != null) {
      for (
        var ii = rangeConnection.port.range[0];
        ii <= rangeConnection.port.range[1];
        ii++
      ) {
  ...
  var __prepareOptions = function (connection, options) {
    ...
    try {
      if (connection != null && JSON.stringify(options) == '{}')
        //empty object
        options = connection;
    } catch (e) {
      //do nothing
  ...
  HappnClient.prototype.__prepareSecurityOptions = function (options) {
    if (options.keyPair && options.keyPair.publicKey)
  ...
  HappnClient.prototype.__prepareSocketOptions = function (options) {
    ...
    if (!options.socket.reconnect.retries)
      options.socket.reconnect.retries = Infinity;
    ...
    if (!options.socket.strategy)
      options.socket.strategy = 'disconnect,online,timeout';
  };

  HappnClient.prototype.__prepareConnectionOptions = function (
    options,
    defaults
  ) {
  ...
  HappnClient.prototype.__prepareOptions = function (options) {
      ...
      for (var overrideName in preparedOptions.plugin) {
        if (preparedOptions.plugin.hasOwnProperty(overrideName)) {
          if (preparedOptions.plugin[overrideName].bind)
            this[overrideName] = preparedOptions.plugin[overrideName].bind(
              this
            );
    ...
    if (preparedOptions.pool) {
      ...
      if (!preparedOptions.poolReconnectDelay)
        preparedOptions.poolReconnectDelay = 0; //no delay by default

      if (preparedOptions.poolType == null)
        preparedOptions.poolType = _this.constants.CONNECTION_POOL_TYPE.RANDOM;

      if (preparedOptions.poolReconnectAttempts == null)
        preparedOptions.poolReconnectAttempts = 4;

      preparedOptions.pool = preparedOptions.pool.map(function (connection) {
        return _this.__prepareConnectionOptions(
          connection.config ? connection.config : connection,
          preparedOptions
        );
      });
    ...
    if (preparedOptions.allowSelfSignedCerts)
    ...
    if (preparedOptions.loginRetryInterval == null)
      preparedOptions.loginRetryInterval = 5000; // five seconds apart
    ...
    if (preparedOptions.defaultVariableDepth == null)
      preparedOptions.defaultVariableDepth = 5;
    ...
    _this.__getConnection = function (callback) {
      ...
      try {
        if (_this.options.pool) {
          ...
          if (
            _this.options.poolReconnectAttempts > 0 &&
            _this.__poolAttempts > _this.options.poolReconnectAttempts
          ) {
            return callback(new Error('pool reconnection attempts exceeded'));
          }

          if (_this.__possible.length == 0) {
            //what is possible is now zero, start all over again until we get a connection or we have reconnected successfully
          ...
          if (
            _this.options.poolType ==
            _this.constants.CONNECTION_POOL_TYPE.RANDOM
          )
            possibleIndex = Math.floor(Math.random() * _this.__possible.length);

          if (
            _this.options.poolType ==
            _this.constants.CONNECTION_POOL_TYPE.ORDERED
          )
            possibleIndex = 0;
  ...
  HappnClient.prototype.__initializeEvents = function () {
    ...
    _this.onEvent = function (eventName, eventHandler) {
      ...
      if (!_this.state.eventHandlers[eventName])
        _this.state.eventHandlers[eventName] = [];

      _this.state.eventHandlers[eventName].push(eventHandler);

      return (
        eventName + '|' + (_this.state.eventHandlers[eventName].length - 1)
      );
    };
  ...
  HappnClient.prototype.getResources = function (callback) {
    ...
    if (typeof Primus == 'undefined')
      _this.getScript(_this.options.url + '/browser_primus.js', function (e) {
        if (e) return callback(e);
  ...
  HappnClient.prototype.__encryptLogin = function (parameters, publicKey) {
    return {
      ...
      loginType:
        parameters.loginType != null ? parameters.loginType : 'password'
    };
  };
  ...
  HappnClient.prototype.__encryptPayload = function (message) {
    ...
    if (self.session.protocol === 'happn_4') {
      ...
      payload = crypto.symmetricEncryptObject(message, self.session.secret);
    }
  ...
  HappnClient.prototype.__decryptPayload = function (message) {
    ...
    if (self.session.protocol == 'happn_4') {
      ...
      payload = crypto.symmetricDecryptObjectiv(
        message,
        self.session.secret,
        iv
      );
    } else {
      payload = crypto.symmetricDecryptObject(message, self.session.secret);
    }
  ...
  HappnClient.prototype.__doLogin = function (loginParameters, callback) {
    ...
    if (
      !_this.options.loginRetryInterval ||
      typeof _this.options.loginRetryInterval != 'number'
    )
  ...
  HappnClient.prototype.login = Promisify(function (callback) {
    ...
    _this.__performSystemRequest(
      ...
      function (e) {
        ...
        _this.__performSystemRequest('describe', null, null, function (
          e,
          serverInfo
        ) {
          if (e) return callback(e);
          ...
          if (_this.serverInfo.secure) {
            if (!loginParameters.token && !loginParameters.username)
              return callback(
                new Error(
                  'happn server is secure, please specify a username or token'
                )
              );
            ...
            if (!loginParameters.password && !loginParameters.token) {
              ...
              //we have a username, publicKey and password
                ...
                return callback(
                  new Error('happn server is secure, please specify a password')
                );
            }

            if (
              _this.serverInfo.encryptPayloads ||
              loginParameters.loginType === 'digest'
            ) {
              _this.__ensureCryptoLibrary(function (e) {
                ...
                if (!_this.options.privateKey || !_this.options.publicKey) {
                  if (loginParameters.loginType === 'digest')
                    return callback(
                      new Error(
                        'login type is digest, but no privateKey and publicKey specified'
                      )
                    );
                ...
                _this.__prepareLogin(loginParameters, function (
                  e,
                  preparedParameters
                ) {
                  if (e) return callback(e);
  ...
  HappnClient.prototype.authenticate = Promisify(function (callback) {
    ...
    _this.__getConnection(function (e, socket) {
      ...
      _this.socket.on(
        'reconnect timeout',
        _this.handle_reconnect_timeout.bind(_this)
      );

      _this.socket.on(
        'reconnect scheduled',
        _this.handle_reconnect_scheduled.bind(_this)
      );
  ...
  HappnClient.prototype.handle_reconnect_scheduled = function (opts) {
    ...
    if (_this.options.pool) {
      ...
      _this.reconnect(function (e) {
        if (e)
          _this.handle_error(
            new Error('pooled subscription reconnection failed', e)
          );
      });
    }
  };
  ...
  HappnClient.prototype.__performDataRequest = function (
    path,
    action,
    data,
    options,
    ...
    if (this.status != STATUS.ACTIVE) {
      ...
      if (this.status === STATUS.CONNECT_ERROR)
        errorMessage = 'client in an error state';
      if (this.status === STATUS.ERROR)
        errorMessage = 'client in an error state';
      if (this.status === STATUS.UNINITIALIZED)
        errorMessage = 'client not initialized yet';
      if (this.status === STATUS.DISCONNECTED)
        errorMessage = 'client is disconnected';
    ...
    if (this.serverInfo.encryptPayloads)
      message = this.__encryptPayload(message);
    if (callback)
      this.__requestCallback(
        message,
        callback,
        options,
        message.eventId,
        path,
        action
      ); // if null we are firing and forgetting

    this.socket.write(message);
  };

  HappnClient.prototype.__performSystemRequest = function (
    action,
    data,
    options,
    callback
  ...
  HappnClient.prototype.increment = Promisify(function (
    path,
    gauge,
    increment,
    handler
    ...
    if (isNaN(increment))
      return handler(new Error('increment must be a number'));
  ...
  HappnClient.prototype.__reattachListeners = function (callback) {
    ...
    _this.utils.async(
      ...
      function (eventPath, index, next1) {
          ...
          function (listener, index, next2) {
            ...
            _this._remoteOn(eventPath, parameters, function (e, response) {
              if (e)
                return next2(
                  new Error(
                    'failed re-establishing listener to path: ' + eventPath,
                    e
                  )
                );
  ...
  HappnClient.prototype.reconnect = function (options) {
    ...
    _this.authenticate(function (e) {
      ...
      if (e) {
        if (e.message && e.message.indexOf('api request timed out') == 0)
  ...
  HappnClient.prototype.__attachPublishedAck = function (options, message) {
    ...
    if (typeof options.onPublished != 'function')
      throw new Error('onPublish handler in options is missing');
  ...
  HappnClient.prototype.handle_ack = function (message) {
    ...
    if (_this.state.ackHandlers[message.id]) {
      if (message.status == 'error')
        return _this.state.ackHandlers[message.id].handle(
  ...
  HappnClient.prototype.handle_publication = function (message) {
    if (message.encrypted) {
      if (message._meta && message._meta.type == 'login')
    ...
    if (message._meta && message._meta.type == 'ack')
      return this.handle_ack(message);

    if (Array.isArray(message))
      return this.handle_response_array(null, message, message.pop());
  ...
  HappnClient.prototype.handle_data = function (path, message) {
    ...
    _this.__acknowledge(message, function (acknowledged) {
      ...
      if (_this.state.events[path].length == 1)
        return _this.delegate_handover(
          acknowledged,
          _this.state.events[path][0]
        );
  ...
  HappnClient.prototype.__clearSubscriptionsOnPath = function (
    eventListenerPath
  ) {
    var _this = this;
  ...
  HappnClient.prototype.__clearSecurityDirectorySubscriptions = function (path) {
    ...
    Object.keys(this.state.events).forEach(function (eventListenerPath) {
      if (
        path == eventListenerPath.substring(eventListenerPath.indexOf('@') + 1)
      ) {
  ...
  HappnClient.prototype.__updateSecurityDirectory = function (message) {
    ...
    if (message.data.whatHappnd == 'permission-removed') {
      if (['*', 'on'].indexOf(message.data.action))
        return this.__clearSecurityDirectorySubscriptions(
          message.data.changedData.path
        );
    }

    if (message.data.whatHappnd == 'upsert-group') {
      Object.keys(message.data.changedData.permissions).forEach(function (
        permissionPath
      ) {
        var permission = message.data.changedData.permissions[permissionPath];
        if (
          permission.prohibit &&
          (permission.prohibit.indexOf('on') > -1 ||
            permission.prohibit.indexOf('*') > -1)
        )
  ...
  HappnClient.prototype.__handleSystemMessage = function (message) {
    ...
    if (message.eventKey == 'security-data-changed')
      this.__updateSecurityDirectory(message);

    this.state.systemMessageHandlers.every(function (messageHandler) {
      return messageHandler.apply(messageHandler, [
        message.eventKey,
        message.data
      ]);
    });
  ...
  HappnClient.prototype.__clearListenerState = function (path, listener) {
    this.state.events[path].splice(
      this.state.events[path].indexOf(listener),
      1
    );
    if (this.state.events[path].length == 0) delete this.state.events[path];
    this.state.refCount[listener.eventKey]--;
    if (this.state.refCount[listener.eventKey] == 0)
      delete this.state.refCount[listener.eventKey];
    this.__clearListenerRef(listener);
  };

  HappnClient.prototype.on = Promisify(function (
    path,
    parameters,
    handler,
    callback
    ...
    if (!parameters.event_type || parameters.event_type == '*')
      parameters.event_type = 'all';
    if (!parameters.count) parameters.count = 0;
    if (variableDepth && !parameters.depth)
      parameters.depth = this.options.defaultVariableDepth; //5 by default

    if (!callback) {
      if (typeof parameters.onPublished != 'function')
        throw new Error(
          'you cannot subscribe without passing in a subscription callback'
        );

      if (typeof handler != 'function')
        throw new Error(
          'callback cannot be null when using the onPublished event handler'
        );
      callback = handler;
    ...
    if (!_this.state.refCount[listener.eventKey])
      _this.state.refCount[listener.eventKey] = 0;
    _this.state.refCount[listener.eventKey]++;
    ...
    if (
      ...
      return _this._remoteOn(path, parameters, function (e, response) {
        ...
        if (listener.initialCallback)
          return callback(null, listener.id, response);
  ...
  HappnClient.prototype._offListener = function (handle, callback) {
    ...
    if (!_this.state.events || _this.state.events.length == 0)
      return callback();
  ...
  HappnClient.prototype.off = Promisify(function (handle, callback) {
    ...
    if (typeof handle != 'number')
      return callback(new Error('handle must be a number'));
  ...
  HappnClient.prototype.__connectionCleanup = function (options) {
    try {
      if (this.socket) {
        var _this = this;
        if (options && options.revokeSession)
          return _this.revokeSession(function (e) {
            if (e) {
              _this.log.warn(
                'socket.end failed in client, revoke session failed: ' +
                e.toString()
              );
            }
