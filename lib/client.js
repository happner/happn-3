...
(function() {
  ...
  HappnClient.prototype.__prepareSocketOptions = function(options) {
    ...
    options.socket.strategy =
      options.socket.reconnect.strategy || options.socket.strategy || 'disconnect,online,timeout';
  };
  ...
  HappnClient.prototype.__connectionCleanup = function(options) {
    try {
      ...
      if (options.revokeToken || options.revokeSession)
        return this.revokeToken(function(e) {
          if (e)
            _this.log.warn('socket.end failed in client, revoke-token failed: ' + e.toString());
          _this.__endSocket();
        });

      if (options.disconnectChildSessions)
        return this.__performSystemRequest('disconnect-child-sessions', null, null, function(e) {
          if (e)
            _this.log.warn(
