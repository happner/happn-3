...
(function() {
  ...
  HappnClient.create = maybePromisify(function(connection, options, callback) {
    ...
    if (typeof options.cookieEventHandler === 'function' && browser) {
      HappnClient.addCookieEventHandler(
        ...
        options.cookieName || 'happn_token',
        options.cookieEventInterval || 1000
      );
    }
  ...
  HappnClient.addCookieEventHandler = function(handler, cookieName, interval) {
    if (typeof handler !== 'function') {
      throw new Error(`argument handler at position 0 null or not a function`);
    }
    if (typeof cookieName !== 'string') {
      throw new Error(`argument cookieName at position 1 null or not a string`);
    }
    const cookieCheckInterval =
      isNaN(interval) || parseInt(interval) < 500
        ? 1000 // if not a number or < 500 set to sane default
        : parseInt(interval);
    const cookieCheckKey = `${cookieName}_${cookieCheckInterval}`;
  ...
  HappnClient.__setupCookieCheckInterval = function(cookieName, interval, handlers, newHandler) {
    ...
    //emit the status back to the handler
    newHandler(HappnClient.__cookieCheckStatuses[cookieName], cookieInstance);
