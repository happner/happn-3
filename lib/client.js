...
(function() {
  ...
  HappnClient.prototype.__connectSocket = function(callback) {
    ...
    socket.on('error', function(e) {
      if (_this.status === STATUS.CONNECTING) {
        ...
        _this.status = STATUS.CONNECT_ERROR;
        _this.__destroySocket(socket, destroyErr => {
          if (destroyErr)
            _this.log.warn(
  ...
  HappnClient.prototype.on = maybePromisify(function(path, parameters, handler, callback) {
    if (typeof parameters === 'function') {
      callback = handler;
      handler = parameters;
      parameters = {};
  ...
  HappnClient.prototype.onAll = maybePromisify(function(handler, callback) {
  ...
  HappnClient.prototype.__destroySocket = function(socket, callback) {
    ...
    socket
      .once('close', () => {
        // wait for next loop cycle to allow for primus cleanup
        setTimeout(() => {
          let destroyError;
          try {
            socket.destroy();
  ...
  HappnClient.prototype.__connectionCleanup = function(options, callback) {
    ...
    if (options.revokeToken || options.revokeSession)
      return this.revokeToken(e => {
        if (e) this.log.warn('socket.end failed in client, revoke-token failed: ' + e.toString());
        this.__destroySocket(this.socket, callback);
      });
    ...
    if (options.disconnectChildSessions)
      return this.__performSystemRequest('disconnect-child-sessions', null, null, e => {
        if (e)
          this.log.warn(
            'socket.end failed in client, disconnect-child-sessions failed: ' + e.toString()
          );
        this.__destroySocket(this.socket, callback);
      });
    this.__destroySocket(this.socket, callback);
