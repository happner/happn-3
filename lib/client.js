...
(function() {
  ...
  HappnClient.prototype.__reattachChangedListeners = function(permissions) {
    ...
    listenerPermPaths.forEach(path => {
      let channels = Object.keys(this.state.events).filter(channel => channel.endsWith(path));
      if (channels) {
        channels.forEach(channelPath => {
          let listeners = this.state.events[channelPath];
          listeners.forEach(listener => {
            this.state.refCount[listener.eventKey] = 0;
          });
          listeners.forEach(listener => {
            if (!this.state.events[channelPath]) return; // We  have deleted this key.
