...
(function() {
  ...
  HappnClient.prototype.__initializeState = function() {
    this.state = {};
    this.state.multiPathEvents = {}
    this.state.multiPathEvents.events = {};
    this.state.multiPathEvents.refCount = {};
    this.state.multiPathEvents.listenerRefs = {};
    this.state.events = {};
    this.state.refCount = {};
  ...
  HappnClient.prototype.__attachSession = function(result) {
    delete result._meta;
    this.session = result;
    if (browser) this.__writeCookie(result, document);
  };
  ...
  HappnClient.prototype.__reattachChangedListeners = function(permissions) {
    ...
    let listenerPermPaths = Object.keys(permissions).filter(
      key => permissions[key].actions.includes('*') || permissions[key].actions.includes('on')
    );
    this.state.refCount = this.state.refCount || {};
    let errors = [];
    if (listenerPermPaths) {
  ...
  HappnClient.prototype.__updateSecurityDirectory = function(message) {
    ...
    if (message.data.whatHappnd === _this.CONSTANTS.SECURITY_DIRECTORY_EVENTS.UPSERT_USER) {
      Object.keys(message.data.changedData.permissions).forEach(function(permissionPath) {
        var permission = message.data.changedData.permissions[permissionPath];
        if (
          permission.prohibit &&
  ...
  HappnClient.prototype.on = promisify(function(path, parameters, handler, callback) {
    ...
    let variableDepth =
      ...
      if (typeof path === 'string' && (path.includes("**") || path.includes("*"))) {
        relevantPaths = this.fetchRelevantPaths()
        if (relevantPaths.length>1 || 
          (relevantPaths.length == 1 && relevantPaths[0] != path))
          return this.processMultipleSubscriptions()
      }
